...
::: r
If you are running this notebook on your own computer, using RStudio, you are running an RMarkdown version of the notebook.  These notebooks are very similar to Jupyter notebooks, but with some differences in the way they work, and their terminology.  For example, code cells are called *code chunks*.  We will often use code cell and code chunk interchangeably.
:::
...
::: r
In what follows, we will assume you are running this notebook with the Jupyter interface that we provide with the textbook for the interactive examples.

We will also note where you will see or do something differently, when you are running the notebook on your own computer, using RStudio.
:::

If you are in the notebook interface (rather than reading this in the
textbook), you will see the Jupyter menu bar near the top of the page, with
headings "File", "Edit" and so on.

::: r
RStudio has its own menu, with "File", "Edit", etc.
:::
...
::: r
RStudio has its own tool bar, with a Run icon, to run a code chunk (code cell).
:::
...
We are using the Jupyter notebook for our interactive (web) examples.  Although
you can use the Jupyter notebook on your own computer, we recommend you use the
standard [RStudio](https://posit.co/products/open-source/rstudio) desktop
application.  It works in similar way to Jupyter, but with some variation.  You will find many good tutorials for RStudio online; at time of writing [this DataCamp tutorial](https://www.datacamp.com/tutorial/r-studio-tutorial) was a good place to start.

As you will see in the tutorial linked above, you will need two software
packages on your computer to run RStudio.  These are:
