...
The answer is somewhere between 5 and 6%.  The code clearly follows the description step by step, but it is also quite slow.  If we can improve the code, we may be able to do our simulation with more cars, and get a more accurate answer.
...
Thereafter, we can still look into different—sometimes more elegant or accurate—ways of modeling and solving the problem.
...
We often count how commonly events occur: "it rained 4 out of the 10 days".
...
Point being that the denominator doesn't matter when calculating ratios, so we
...
of having an auto accident. It is known that there are two classes of
...
respectively ((1/4)/(1/4 + 1/2) vs (1/2)/(1/4 + 1/2)). Given this prior information, Fisher supposed that the
...
Original instructions XXX TODO REWRITE XXX:
...
# The mouse will now be mated with a brown mouse, producing 7 offspring.
# we create an Mx7 array to store those in.
babies = np.zeros((M, 7), dtype=object)

for i in range(M):
    mouse = test_mice[i]
    
    if mouse == 'BB':
        # If the mouse is 'BB', we are assured that all its children
        # will be 'Bb'
        babies[i, :] = 'Bb'

    elif mouse == 'Bb':
        # If the mouse is 'Bb', then we refer to the middle row of the table
        # to see what its offspring are likely to be.
        babies[i, :] = np.random.choice(['Bb', 'bb'], p=[0.5, 0.5], size=7)

# Find out which mice produced 7 black offspring (we were told that this
# was the outcome of the experiment, so we are only interested in those
# cases).
all_babies_with_B = np.all(babies == 'Bb', axis=1)

# Find the parents' genetic kinds
parents = test_mice[all_babies_with_B]

# Calculate what fraction of parents were 'BB' vs 'Bb'
parents_BB = (parents == 'BB')
parents_Bb = (parents == 'Bb')
N_B = np.count_nonzero(all_babies_with_B)
...
analytic method (see e.g. Box and Tiao, Appendix A1.1) make it almost impossible for the statistician to fully understand the procedure from beginning to end; if one is interested in insight, the simulation
