...
# Create a new array in which to stores what a mechanic says
# about the car: 'approved' or 'not approved'
mechanic_says = np.empty_like(cars, dtype=object)

# For each working car, generate a mechanic mechanic_says/declared faulty
...
# For each faulty car, generate a mechanic mechanic_says/declared faulty
# label with the given probabilities
...
There is about a 5% chance of a car being broken after it has been
approved by a mechanic.

### Calculation without simulation

Often, we use simulations to give us a sense of what the right answer
to a problem should be. Thereafter, we can look into different—sometimes more elegant or accurate—ways of modeling and solving it.

Let's examine the following tree diagram of our car selection:
...
Notation-wise, it is a bit easier to calculate these sums using fractions rather than percentages:
...
We also count how commonly events occur: "it rained 4 out of the 10 days".

An extension of this idea is to *predict* the probability of an event occurring, based on what we had seen in the past.
We can say "out of 100 observations, condition X occurred 20 times; therefore, I  estimate that the probability of X occurring, $P(X)$, is 20/100".
The more observations we have, the better our probability estimates become.
...
To calculate that, we'd first count the number of red cars, then the number of those red cars that are also broken, then calculate the ratio: `red_cars_broken / red_cars`.
...
I.e., the denominator doesn't matter when calculating ratios, so we
...
This is read as "the probability that the car is broken given that the car is red".
Such a probability is known as a *conditional probability*.  We discuss these in more details in **TODO: ADD REFERENCE TO CONDITIONAL PROBABILITY**.

In our original problem, we ask what the chance is of a car being broken given that a mechanic approved it.  As discussed under "Ratios of proportions", it can be written as:
...
We will use `P+`/`P-` to indicate COVID positive or negative, and `T+`/`T-` to indicate positive or negative test results.
...
$$ \mathrm{ P(C+ | T-) = P(C+ \cup T-) / P(T-) } $$

We see from the tree diagram that $P(C+ \cup T-) = P(T- | C+) * P(C+) = .4 * .015 = 0.006$ (** TODO: ADD REFERENCE TO SUMMATION OF MUTUALLY EXCLUSIVE PROBABILITIES **).

We observe that $P(T-) = P(T- \cup C-) + P(T- \cup C+)$, i.e. that we can obtain a negative test result through two paths, having COVID or not having COVID.  We expand these further as conditional probabilities:
...
We can now calculate our answer:
...
test_result[~person_has_covid] = np.random.choice(
    ...
    size=nr_without_covid
)

covid_status_negative_test = person_has_covid[~test_result]
nr_with_covid_and_negative_test = np.count_nonzero(covid_status_negative_test)
nr_with_negative_test = len(covid_status_negative_test)

k = nr_with_covid_and_negative_test / nr_with_negative_test

print(k)
```

This gives around 0.006; that matches our calculated answer!
