...
Start by examining the following tree diagram:
...
There are two approaches we could take: either compute our chances
based on the numbers in the tree directly, or simulate the situation.
Especially in complex scenarios, the latter is often easier to
implement and gives us a better sense of the various outcomes that
might arise.  But the prior method is a quick way of getting a single
estimate.

Let's walk through both approaches.

**Calculation using the odds provided:**

We want to know, of all the cars approved by a mechanic, how many are faulty:

$$ \frac{\mathrm{cars_{\mathrm{faulty}}}}{\mathrm{cars}_{\mathrm{approved}}} $$

We saw that there are two paths in the tree that lead to a mechanic
approving a car, and we can calculate the fraction of cars that end up so:

1. Of all cars, 30% are faulty.  Of these, 10% are approved by a mechanic.  That is,
   30% * 10% = 3% of all cars.
2. Of all cars, 70% work.  Of these, 80% are approved by a mechanic.  That is,
   70% * 80% = 56% of all cars.

The percentage of faulty cars, out of approved cars, is therefore:

$$ 3% / (56% + 3%) = 5.08%. $$

Notation-wise, it is a bit easier to calculate these sums as fractions instead of as percentages:
...
1. We note that we are only interested in outcomes where the mechanic says the cars are OK.

2. We start with N cars.  For each car, generate a label of either
   "faulty" or "working" with 0.3 and 0.7 probabilities respectively.

3. For each faulty car, we generate one of two labels, "mechanic
   approved" or "mechanic failed", with probabilities of 0.1 or 0.9
   respectively.

4. For each working car, generate one of two labels, "mechanic
   approved" or "mechanic failed", with probabilities 0.7 and 0.3
   respectively.

5. Out of all cars "mechanic approved", count how many are "faulty".
...
# Count how many cars are working and how many are faulty
N_working = np.count_nonzero(cars == 'working')
N_faulty = N - N_working

# Create a new array in which to store mechanic approved (True)
# or mechanic declared faulty (False)
approved = np.zeros_like(cars, dtype=bool)

# For each working car, generate a mechanic approved/declared faulty
# label with the given probabilities
approved[cars == 'working'] = np.random.choice(
    [True, False], p=[0.8, 0.2], size=N_working
)

# For each faulty car, generate a mechanic approved/declared faulty
# label with the given probabilities
approved[cars == 'faulty'] = np.random.choice(
    [True, False], p=[0.1, 0.9], size=N_faulty
)

# Select out only the cars that are faulty and were approved
faulty_but_approved = np.logical_and(cars == 'faulty', approved)
...
In this experiment, we use 100000 cars, and the results are very
similar to calculating chances directly.  Say, however, that you were
in the situation where the yard only had 100 cars.  Are the odds still
around 5% of getting a faulty car, if you buy one approved by a
mechanic?

Since we have a simulation of the situation, we can gain further insight:
...
def buying_risk(N):
    ...
    return N_faulty_but_approved / N_approved


experiments = 1000
cars = 100

risk = np.array([buying_risk(cars) for i in range(experiments)])

print(f'Risk higher than 10% in {np.count_nonzero(risk >= 0.1) / experiments * 100:.2}% of experiments')

plt.hist(risk)
plt.show()
```

Let's call our previous answer, faulty cars out of all approved cars, buying risk.  We see that, from a batch of 100 cars, the buying risk can vary tremendously.  In fact, while the average buying risk is 5%, as expected, in about 6% of cases the risk is higher than 10%!
...
At our mechanic's yard, we ask "how many red cars here are faulty"?  To calculate that, we first count the number of red cars, then the number of those red cars that are broken, then calculate the ratio: `red_cars_broken / red_cars`.
