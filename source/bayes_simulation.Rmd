...
balls in it, 2 marked `Bb` and 1 marked `BB`. We can now draw, with replacement, from this urn. If we draw `BB`, the
...
color that can be sampled is `Black`. We therefore put a single (or any number, for that matter) black ball in the
urn of `Model 1`- ensuring that we'll always draw a black ball. For the urn of `Model 2` we turn to
...
has two balls, one labelled `Black`, the other `Brown`.
...
a. If we draw `BB` from the first urn, we next draw seven times (with replacement)
   the urn of `Model 1`. We know that we'll only draw black balls, therefore the probability
...
b. If we draw `Bb` from the first urn, we next draw seven times (with replacement)
   the urn of `Model 2`. We know that we'll draw black or brown balls, each with probability 1/2. The probability
   of drawing 7 black balls in succession is therefore $(1/2)^7$. Again with the same abuse of notation, we have
