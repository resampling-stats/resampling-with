...
1. *You are a doctor* trying to develop a treatment for blood clots in COVID19.
   Currently you are working on a medicine labeled CCC. You have data from
   patients to whom medicine CCC was given. You want to judge on the basis of
   those results whether CCC really reduces blood clots or whether it is no
...
2. *You are an environmental scientist* monitoring levels of phosphorus
   pollution in a lake [@zhou2000long].  The phosphorus levels have all been
   been below 70 units until 1990, but they have all been above 70 since then.
   Does this increase indicate some important change or could it be due to some
   chance ordinary variation in the yearly levels?
...
1. *Should you (the researcher) advise doctors to prescribe medicine CCC* for
   COVID19 patients, or, should you (the researcher) continue to study CCC
   before releasing it for use? A related matter: should you and other research
   ...
   to some other promising line of research? These are just two of the possible
   decisions that might be influenced by the answer to the question about the
   probability that medicine CCC treats blood clots in COVID19.
...
4. *Should we invest in a search for new causes of phosphorus pollution* as a
   result of the recent measurement from the lake?  If the causes have not
   changed, and the higher recent values were just the result of ordinary
   variation across the years, we could end up investing time and money in a
   search where it could better have been spent elsewhere.
...
numbers we are interested in.  For example, here are the yearly measures of
"soluble reactive phosphorus" (SRP) from a Lough Erne — a lake in Ireland.
...
```{r, yearly-srp, echo=FALSE}
ketable(py$yearly_srp, caption = "Soluble Reactive Phosphorus in Lough Erne")
```

We may well want to *summarize* this set of SRP measurements.
For example, we could add up all the SRP values to give the *total*.  To
describe the center of this collection of numbers, we could divide the total by
the number of measurements, to give the *average* SRP level.  Or we could
measure the spread of the values by finding the *minimum* and the *maximum*.
All these numbers are *descriptive statistics*.
...
```{r, srp-stats, echo=FALSE}
ketable(head(py$srp_stats), caption = "Statistics for SRP levels")
```

These numbers are *descriptive* statistics, because they are summaries that
*describe* the collection of SRP measurements.
...
For example, imagine we have measured a SRP value of 120 for 1998, and we
wonder whether this value is a sign that something has changed in the
environment around the lake.  We could use methods that you will see in this
book to calculate a probability value that is the answer to "What are the
chances that we will see an SRP value of 120 or greater given the collection of
values we have seen up until now?".
...
or (b) you may not, come up with an important general cure for clotting in
viral infections within, say, the next 3 years. If you do come up with such a
general cure, of course it will have very great social benefits. Furthermore,
(c) if you decide not to do further research on CCC now, you can direct your
...
cure for some relatively infrequent causes of blood clotting. Those three
possibilities have different social benefits. The probability that medicine CCC
really has some curative effect on blood clotting, as judged by your prior
research, obviously will influence your decision on whether or not to do more
research on medicine CCC. But that judgment about the probability is only one
part of the overall web of consequences and evaluations that must be taken into
account when making your decision whether or not to do further research on
medicine CCC.
...
1. What will be the result of more than a hundred million Americans voting for
...
2. To assess, on the basis of the data on the prices of a single brand of
   liquor in 42 states, 16 of which have liquor distribution systems run by
   state government and 26 run privately, whether prices will be higher in a
   state newly admitted to the union if it chooses the private system; or
