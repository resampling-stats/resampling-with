...
::: r
Before we start, we need to return to the `c()` function in R: @sec-r-c.
:::
...
::: r
We have already seen R's `c()` (concatenation) function in action (@sec-r-c).
For example, we use `c(1, 2, 3)` to *concatenate* the single numbers 1, 2, and
3 into a combined (concatenated) *vector* with elements 1, 2, 3:
...
:::{#nte-modules-submodules .callout-note}
