...
We use the following standard convention to import that the `pyplot` part of
...
modules-attached-to-modules are called *submodules*.  You have already see us
use the `random` submodule attached to the Numpy module:
...
We have created random number generators by using the `default_rng()` function
from the `random` submodule.
...
The `pyplot` module of Matplotlib has many useful functions for making and displaying plots.
:::
<!---
...
[`plt.hist`]{.python}[`hist`]{.r} has calculated some suitable intervals
(*bins*) to divide up the range of values, and then counted how many values in
`prices` fall into each interval (bin).

You will notice that [`plt.hist`]{.python}[`hist`]{.r} has sent back some
results from the process of making the histogram.

The first result of interest to us is the definition of the intervals (bins) into which the histogram has divided the range of the `prices` values.
...
Think of these values as the 10 values that start each of the 10 bins, followed
...
The first value that comes back in the list of results is the array of counts of values in `prices` that fall within each bin.
...
This means that 1 value from `prices` fell in the range 3.8 up to (not
including) 3.95, 2 values fell in the range 3.95 up to (not including) 4.1, and
so on.

Notice that the counts correspond to the heights of the bars on the histogram,
...
number of bins, by sending a number for the `bins` argument to `plt.hist`.  For
...
If we run `plt.hist` without collecting the values that come back, Jupyter (as
usual) will show the values as the result of the cell.
...
Here we see that the result that came back is a list with three values.  As you saw before, the first value is the array with the counts for each of the (by default) 10 bins.  The second is the array with the bin edges (10 left edges and last right edge).  The last is a reference to the values that make up the graphical display; this is advanced, and we won't look at these any further in this book.


It can be distracting to see the counts, bin edges and graphical display values along with the plot, so from now on we will suppress Jupyter's default behavior of showing the results that came back from the `plt.hist` function call by adding a semi-colon at the end of the function call, like this:

```{python}
plt.hist(prices);  # Note the semi-colon
```

The semi-colon is a standard indicator to Jupyter that it should not display the results that came back from the function call.   To repeat, we use it with plotting functions to suppress the various values that come back from these functions, as they are usually not of immediate interest.
...
to mean "get the value called `counts` that is attached to the `hist_res`
value."
...
decided it would make 9 bins, with the edges you see in the `breaks` value of
`hist_res`.

```{r}
# Bin edges
bin_edges <- hist_res$breaks
bin_edges
```

Think of the `breaks` values as: the value that starts the first bin, followed
by the 9 values that end each of the 9 bins.[^r-hist-right]

[^r-hist-right]: In fact, you can customize the calculation and meaning of the
    `breaks` values using various arguments to the `hist` function.  See the
    documentation of `hist` for more details.

The `breaks` values show that the first bin was from (including) 3.8 up to,
...
(including) 5.4.  Notice there are 10 edges (breaks), forming 9 bins.
...
This means that 3 values from `prices` fell in the range 3.8 through 4.0, 6 values fell in the range from (not including) 4.0 through 4.2, and
so on.

Notice that the counts correspond to the heights of the bars on the histogram,
so the first bar has height 3, the second bar has height 6, and so on.

By default, `hist` uses Sturge's rule to decide on the number of bins.  You can also *suggest* to R that it uses another number of bins, by sending a number for the `breaks` argument to `hist`.  For example, you might want to suggest 20 bins
...
number of bins to something close to the suggestion and that gives
a pleasant-looking histogram.   In fact, R responded to the suggestion of 20 by
giving 15 bins:
...
(exclusive) right-hand edges.

```{r}
bin_edges_20 <- results_20$breaks
bin_edges_20
```

We can also specify our own edges, in order to bypass `hist`s default
