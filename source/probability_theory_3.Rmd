...
PDF](https://resample.com/content/text/11-Chap-7.pdf).
...
for (i in 1:10000) {
    ...
    # in duplicates, we would have put in sum(counts > 0).
...
for i in range(10000):
    ...
    children = rnd.integers(1, 3, size=4)
...
for (i in 1:10000) {
    ...
    children <- sample(1:2, size=4, replace=TRUE)
...
the computer solution we would have used the command
...
coins more tedious. But with {{< var lang >}} it is not at all more difficult.
...
The only change we need to the above, for the 52 to 48 case, is:

```{python opts.label="py_ed"}
# Generate 4 numbers at random, from 1 to 100.
children = rnd.integers(1, 101, size=4)

# Let "1" through "48" represent girls, count the number of girls, and put the
# result in b.
b = np.sum(children <= 48)
```
...
5.  Count the proportion "yes" out of the four hundred throws. That
    ...
    4.  Repeat step 2 four hundred times.
        ...
        "bball" should be easy: To create 10000 samples, we start with a
        ...
        loop. The final step is to search the vector z after the 10000
...
for i in range(10000):
    ...
    if b == 1:
        ...
        c = np.sum(np.logical_and(a >= 2, a <=7))
...
1.  Fill a bucket with:
    ...
    * 1 balls marked "2.4", for the heads.

2.  Pick a ball from each bucket, calculate the sum, and replace the balls.
...
statement "GENERATE 1 1,2 C." After you study that, go back and notice the
REPEAT 200 loop that describes the procedure for flipping a coin 200 times.
Finally, note how the REPEAT 10000 loop simulates 10000 games, each game
consisting of 200 coin flips.
...
```{r fig-drunk-walk, echo=FALSE, fig.align="center", fig.cap="Drunk random walk"}
...
Crown whiskey in the two sets of states:

```{python echo=FALSE}
import numpy as np  # For R edition, Numpy will not have been imported.
import pandas as pd
priv = np.array([
...
with_means = pd.concat([df,
                        blank_row,
                        means.rename('Means').to_frame().T])
with_means = with_means.round(2).astype(object).fillna('')
means_tab = with_means.to_markdown(tablefmt="grid")
```

`r py$means_tab`

```{python fig-whiskey-hist, echo=FALSE, fig.align="center", fig.cap="Whiskey prices by state category"}
...
histo(axes[2], govt, 'Private and government', gov_col, alpha=0.5);
```
...
sixteen state-ownership prices. The following computer program performs the
