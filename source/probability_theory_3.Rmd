...
# p=0.19.
rnd.choice(['girl', 'boy', 'not-recorded'], size=10, p=[0.4, 0.41, 0.19])
```

```{r, opts.label="r_ed"}
# Draw 'girl' with probability (p) 0.4, 'boy' with p=0.41, 'not-recorded' with
# p=0.19.
sample(c('girl', 'boy', 'not-recorded'), size=10, prob=c(0.4, 0.41, 0.19),
       replace=TRUE)
```
...
You might wonder how {lang does this trick of choosing the elements with
different probabilities.

One way of doing this is to use *uniform* random numbers.  These are random
numbers that, by default, can be any floating number from 0 through 1.
...
range 0 through 1, there is a 50% chance is it less then 0.5 and a 50% chance
it is greater than 0.5 (technically of course it could be *exactly equal* to
0.5, but this is vanishingly unlikely, so we will ignore that for now).
...
### More precise simulation for the daughters problem
