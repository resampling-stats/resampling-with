...
## Technical aside: logical operators {#sec-logical-operators}

This section continues our programme of expanding the range of {{< var lang >}} features that you can use to clear code.  As we introduce each feature, we will use them in the following examples.

We are about to use the
[`or`]{.python}['|' (*or*)]{.r}
operator in {{< var lang >}}.  An operator, for our purposes is a symbol or
word that sits between two values, and that tells {{< var lang >}} how to combine these values.

For example `+` is an operator.  When `+` sits between two numbers in {{< var lang >}} code, {{< var lang >}} interprets this to mean "take the two numbers either side, and make a new number that is the result of doing arithmetic addition of the two numbers:
...
`+`, `-`, `/` and `*` are all examples of operators that do *arithmetic* on
the numbers to either side — they are *arithmetic* operators.
...
[`or`]{.python}['|' (*or*)]{.r}.
[`or`]{.python}['|']{.r} is a *logical* operator.  It is a logical operator because it does not operate on *numbers* (as arithmetic operators do), but on *logical* or *Boolean* values — values that can be either {{< var true >}} or {{< var false >}}.

For example, here we use
[`or`]{.python}['|' (*or*)]{.r}.
to combine a {{< var true >}} value (on the left) with a {{< var false >}} value (on the right).  It gives a result — {{< var true >}}.
...
[`or`]{.python}['|' (*or*)]{.r} applies a very simple rule: if *either* the left-hand (LH) *or* the right-hand (RH) values are {{< var true >}}, then
[`or`]{.python}['|' (*or*)]{.r} evaluates to {{< var true >}}.  Only if
neither of the LH or the RH value is {{< var true >}}, does it return {{< var
false >}}.
