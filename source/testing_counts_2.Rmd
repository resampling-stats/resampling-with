...
### Example: Do Any of Four Treatments Affect Sex Ratio in Fruit Flies?
...
experiment in Example 15-1. Instead, we must consider that our benchmark
universe is composed of *four sets* of twenty trials, each trial having a 50-50
probability of being male. We can consider that our previous trials 1-4 in
Example 15-1 constitute a *single* new trial, and each subsequent set of four
previous trials constitute another new trial. We then ask how likely a new
trial of our sets of twenty flips is to produce *one* set with fourteen or more
of one or the other sex.

Let us make the procedure explicit, but using random numbers instead of
coins this time:

* **Step 1.** Write "male" and "female" on two pieces of paper, put them in
  a hat.
* **Step 2.** Pick a piece of paper at random from the hat, write down whether
  it was "male" or "female"; replace the piece of paper.  Repeat 20 times to get 20 simulated fruit flies.
* *Step 3**
  are 14 or more males, record "yes"; if 13 or less, record "no."
...
* **Step 4.** Calculate the proportion "yes" in the 1000 trials. This
  ...
  a proportion of 50 percent males will produce as many as 14 males in at least
  one of four samples of 20 flies.
...
A key point of the {{< var lang >}} notebook below is that each sample consists
of *four sets* of 20 randomly generated hypothetical fruit flies. And if we
...
for i in range(n_iters):
    # more_than_13 indicates whether we have obtained any trial group with more
    ...
    for j in range(4):
        ...
        n_females = np.sum(flies == 'female')  # Same as: 20 - n_males
        if n_males > 13 or n_females > 13:
            more_than_13 = 1
    # more_than_13 now tells us whether we got a result as extreme as that
    # observed (more_than_13 == 1 if we did, more_than_13 == 0 if not). We must
    # keep track of this result in the results variable, for each experiment.
    results[i] = more_than_13
...
print('Proportion of trials exceeding 13:', kk)
```
...
for (i in 1:n_iters) {
    ...
    for (j in 1:4) {
        ...
        n_females <- sum(flies == 'female')  # Same as: 20 - n_males
        if (n_males > 13 | n_females > 13) {
            more_than_13 <- 1
        }
    }
    # more_than_13 now tells us whether we got a result as extreme as that
...
message('Proportion of trials exceeding 13: ', kk)
```
...
have produced the observed results. The program that follows also GENERATES
the four treatments by using a REPEAT loop, rather than spelling out the
GENERATE command 4 times as above. In RESAMPLING STATS:
...
![Samples of 750 Voters: ](images/21-Chap-17_001.png){#fig-bush-margin}
