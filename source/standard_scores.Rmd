...
[^ordering]: To get ranks we have to be able to sort our values in some
    ...
    for measured data, where the measurement gives a number in the same unit
    for each observation, but may not make sense for other data, such as names
    or labels.  For example, it may ore may not be meaningful to rank names
...
comparable units.  We will explain this more towards the end of the chapter.
