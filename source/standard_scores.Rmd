...
the KOM and AOC districts.

```{python echo=FALSE, eval=TRUE}
kom_income = get_var('kom_income')
aoc_income = get_var('aoc_income')
plt.hist(incomes, bins=25, color='black');
plt.title('Median household income for 2023 congressional districts');
plt.axvline(kom_income, color='red', linestyle=':', label='KOM')
plt.axvline(aoc_income, color='blue', linestyle=':', label='KOM')
plt.legend();
```
...
plt.axvline(kom_income, color='red', linestyle=':', label='KOM')
plt.axvline(aoc_income, color='blue', linestyle=':', label='KOM')
plt.axvline(np.mean(incomes), color='green', linestyle='-', label='Mean')
plt.legend();
```
...
plt.hist(deviations, bins=25, color='black');
plt.title('Deviations of median household income for 2023 congressional districts');
```

The distribution of deviations centers on zero — an in fact the deviations have
a mean of (as near as the computer can accurately calculate) zero:
...
Now let us consider the deviation value for KOM and AOC:

```{python}
print('Deviation for KOM:', deviations[kom_index])
print('Deviation for AOC:', deviations[aoc_index])
```

```{r}
message('Deviation for KOM: ', deviations[kom_index])
message('Deviation for AOC: ', deviations[aoc_index])
```

We have the same problem as before.  Yes, we see that KOM has a positive
...
deviation.  Then we will compare KOM's and AOC's deviations to the average
deviation, to see if they are unusually far from the mean.

You have just seen, above that we can use the literal average of the deviations
(the mean) because the positive and negative deviations will exactly cancel
out, and the mean deviation will always be as near as the computer can
calculate to zero.

To stop the negatives canceling the positives, we can simply knock the negative
signs off all the negative deviations.

This is the job of the {{< var np_or_r >}} `abs` function — where `abs` is
short for *absolute*.  The `abs` function will knock negative signs off
...
or negative*, we can just take the mean of the absolute deviations, like this.
...
spread.  The Mean Absolute Deviation is the average distance (regardless of
positive or negative) of a value from the mean of the values.

We can get an idea of how typical a particular deviation is by dividing by the
deviation by the MAD value, like this:

```{python}
print('Deviation in MAD units for KOM:', deviations[kom_index] / mad)
print('Deviation in MAD units AOC:', deviations[aoc_index] / mad)
```

```{r}
message('Deviation in MAD units for KOM: ', deviations[kom_index] / mad)
...
of the deviations from the mean always gives 0 (with calculation error),
because the positive and negative deviations cancel exactly.
...
Rather confusingly, the field of statistics terms this mean squared deviation
value — the *variance*.  Just to emphasize that naming, let's calculate again
using the "variance" name.
...
The variance is the typical (mean) *squared* deviation.  The units for the
variance, in our case, would be *squared dollars*.  But we are more interested
in the typical *deviation*, in our original units -- dollars rather than
squared dollars.

So we take the square root of the mean squared deviation, to get the *standard
deviation*.  It is the *standard* deviation in the sense that it a measure of
typical deviation, obtained by squaring the deviations, taking the mean, and
...
For example, we can now ask how many standard deviations each of our two
districts of interest are from the mean:

```{python}
print('Deviation in standard deviation units for KOM:',
      deviations[kom_index] / standard_deviation)
print('Deviation in standard deviation units for AOC:',
      deviations[aoc_index] / standard_deviation)
```

```{r}
message('Deviation in standard deviation units for KOM: ',
        deviations[kom_index] / standard_deviation)
...
```{python}
deviations = incomes - np.mean(incomes)
std = np.sqrt(np.mean(deviations ** 2))
deviations_in_stds = deviations / std
deviations_in_stds[:5]
```

```{r}
deviations <- incomes - mean(incomes)
std <- sqrt(mean(deviations ** 2))
deviations_in_stds <- deviations / std
...
*standard scores* or sometimes *z-scores*.  For example, one name for this
procedure is "z-scoring".
...
corresponds to team competing an English Premier League (EPL).  For those of
you with absolutely no interest in sports, this is the league of the top teams
in English football, or soccer to our North American friends.  The columns of
the table give the total number of points at the end of the 2021 season (from
38 games), where the team gets 3 points for a win, and 1 point for a draw.  The
"wages" column gives the estimated total wage bill in thousands of British
Pounds (£1000) for each team.
...
Let's say we own Crystal Palace Football Club. Crystal palace was a bit below
the middle of the league in terms of points. Now we are thinking about whether
we should invest in higher-paid players for the coming season.
...
wage_mean_sq_dev = np.mean(wage_deviations ** 2)
wage_std = np.sqrt(wage_mean_sq_dev)
wage_std
```
...
wage_mean_sq_dev <- mean(wage_deviations ** 2)
wage_std <- sqrt(wage_mean_sq_dev)
wage_std
```

We can write that same calculation in a more compact form as:

```{python}
# More compact version of standard deviation calculation
np.sqrt(np.mean((wages - np.mean(wages)) ** 2))
```

```{r}
# More compact version of standard deviation calculation
sqrt(mean((wages - mean(wages)) ** 2))
```
...
```{python}
point_std = np.sqrt(np.mean((points - np.mean(points)) ** 2))
standard_points = (points - np.mean(points)) / point_std
```

```{r}
point_std = sqrt(mean((points - mean(points)) ** 2))
standard_points = (points - mean(points)) / point_std
```
...
standard scores in wages, we expect an increase of $r$ (`r get_var('r')`) in
points, in standard scores.
