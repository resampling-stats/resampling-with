...
This is a good use for *quantile positions*.  The *quantile position* of
a value tells you where the value ranks relative to the other values, on
a scale from 0 through 1. A quantile position of 0 tells you this is the
lowest-ranking value, and a quantile position of 1 tells you this is the
...
We can calculate the *quantile position* for each rank.  Think of the
low-to-high ranks as being a line starting at 1 (the lowest rank — for the
lowest median income) and going up to 441 (the highest rank — for the highest
median income).
...
Let's plot the ranks and the quantile positions together on the x-axis:
...
ax.plot(hi_to_lo_ranks, sorted_incomes, 
        color='blue', marker='+',
        linestyle='')  # Don't join points with lines
...
Now consider one particular quantile position — 0.5.  The 0.5 quantile position is exactly half-way along the line from rank 1 to rank 441.  In our case this corresponds to rank (441 - 1) / 2 + 1 = 221.
...
This is an interesting value.  Notice that, by the definition of QP, exactly half of the remaining values (after excluding the 0.5 quantile value) have lower rank, and are therefore less   than the 0.5 quantile value.  Similarly exactly half of the remaining values are greater than the 0.5 quantile.   You may recognize this as the *median* value.  And this fact Numpy / R have 
