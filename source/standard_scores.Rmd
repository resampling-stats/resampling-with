...
More generally, we can translate the low-to-high ranks to QPs with:
...
```{r}
message('Quantile position for rank 221: ', quantile_positions[221])
```

The value corresponding to the QP of 0.5 is the *quantile value*, or just
*quantile* for short:

```{python}
# Quantile value for 0.5
print('Quantile value for QP of 0.5:', sorted_incomes[221])
```

```{r}
# Quantile value for 0.5
message('Quantile value for QP of 0.5: ', sorted_incomes[221])
```

In fact we can ask {{< var lang >}} for this *value* (quantile) directly, using
{{< var np_or_r >}}'s `quantile` function:
...
```{r}
quantile(sorted_incomes, 0.5)
```

We've used the sorted values above, but the `quantile` routine will do the
sorting for us, internally, so we can also use the unsorted values:
...
may recognize this as the *median* value.  The median is such a common quartile
value that Numpy / R have a `median` function as a shortcut for
...
qp25_index = qp25_rank - 1
print('0.25 quantile value:', sorted_incomes[qp25_index])
print('0.25 quantile value using np.quantile:', np.quantile(incomes, 0.25))
```
...
Now let's think about the 0.01 quantile.  We don't have a value exactly
corresponding to this QP, because there is no rank exactly corresponding to the
QP of 0.01:

```{python}
qp001 = (441 - 1) * 0.01 + 1
# Not a whole number - no exactly corresponding rank.
qp001
```
...
Let's have a look at the first 10 ranks / QPs, and incomes:
