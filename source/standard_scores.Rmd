...
[^ordering]: To get ranks we have to be able to sort our values in some
    reasonable way, from high to low or low to high.  This usually makes sense
    for measured data, where the measurement gives a number, but may not make
    sense for other data, such as names or labels.  For example, it may ore may
    not be meaningful to rank names alphabetically.  As usual, this is a matter
    of judgment.

If We can derive quantile positions from ranks.  These are values from 0 through
...
that shows this pattern in the US congress. The table lists the top 20
districts of the 2023 congress by the median income of the households in that
...
For example, the median household income of KOM's district is 77205 (dollars).
...
easier to read and understand than `7.7205e41`.  The notation comes into its
