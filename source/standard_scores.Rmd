...
# Ranks, quantile positions and Standard Scores
...
values.  You will see what we mean later in the chapter.

Ranks and quantile positions give an idea whether the measure is high or low compared
...
This is a good use for *quantile positions*.  The *quantile* of a value tells
you where the value is, in the rankings, on a scale from 0 through 1.
A quantile position of 0 tells you this is the lowest-ranking value, and
a quantile position tells you this is the highest-ranking value.

To see how this works, start with rank *from lowest to highest*.  That is, the
lowest value will get rank of 1, and — in our case — the highest value will get
a rank of 441.

This is just the opposite of what we have at the moment, which is the ranks
from highest median income (rank is 1) to lowest median income (rank is 441).


We can get the low-to-high ranks by using the
[`rank`]{.r}[`rankdata`]{.python} function
[from R]{.r}
[from the Scipy statistics library.]{.python}.


::: python
:::{.callout-note}
## Introducing `scipy.stats`

Up until now we have used two big Python libraries for numerical analysis.
These are Numpy (`numpy`), that we have used throughout the book, and Pandas,
that we used in @sec-pandas-intro.

Here we introduce the third major library for numerical analysis in Python,
called Scipy.  Scipy is a large collection of scientific Python routines,
building on top of Numpy.

Scipy has many parts (or "sub-packages"), for a range of scientific
applications, but the part that interests us here is the statistics sub-package
of Scipy, called `scipy.stats`.

We will import that sub-package directly into Python with:

```{python}
# Import the Scipy statistics sub-package.
# Rename to "sps" for short
import scipy.stats as sps
```

In particular, we are going to use the `rankdata` function from `scipy.stats`.
With the import above, we can get to that function with `sps.rankdata`.

:::
:::

We can get the low-to-high ranks for the `incomes` values with:

```{python}
lo_to_hi_ranks = sps.rankdata(incomes)
# Show the first five values.
lo_to_hi_ranks[:5]
```

```{r}
lo_to_hi_ranks <- rank(incomes)
# Show the first five values.
lo_to_hi_ranks[1:5]
```

:::{.callout-note}
## Or ... get the low-to-high ranks from high-to-low ranks

We are in the unusual position that we already have the high-to-low ranks in
the `ranks` variables.  With these, we can get the low-to-high ranks by
subtracting to the high-to-low ranks from 442, so that 441 becomes 1 and 1
becomes 441:

```{python}
again = 442 - hi_to_lo_ranks
again[:5]
```
...
Now we have the low-to-high ranks, we can calculate the *quantile* for each
value.  Think of the low-to-high ranks as being a line starting at 1 (the
lowest rank — for the lowest median income) and going up to 441 (the highest
rank — for the highest median income).

The quantile position corresponding to any particular rank tells you how far
along this line the rank is.  Notice that the length of the line is the
distance from the first to the last value, so 441 - 1 = 440.

So, if the rank was 1, then the value is at the start of the line.  It has got
0 / 440 of the way along the line, and the quantile position is 0. If the rank
is 441, the value is at the end of the line, it has got 440 / 440 of the way
along the line and the quantile position is 1.

Now consider the rank of 100.  It has got $\frac{(100 - 1)}{440}$ of the way
along the line, and the quantile position is `r round(99 / 440, 2)`.

More generally, we can translate the low-to-high ranks to quantile positions with:
...
The quantile positions for KOM and AOC tell us where their districts are in the
...
If we multiply the quantile position by 100, we get the *percentile position*.
...
The *median* is the value corresponding to the quantile position of 0.5 — or,
equivalently — the percentile position of 50.[^median-complications]

[^median-complications]: It is true that median is the value at the 50^th^
    percentile position, but we will only have value at that exact position
    if we have an odd number of values, as we do in the congress income
    example.  Say we have $n$ values (in our case $n = 441$. If $n$ is *odd*,
    then the length of the line from 1 to $n$ is *even*, the 50th percentile
    position $\frac{n}{2}$ is a whole number, and corresponds to middle-ranked
    value in the data. If $n$ is even, the length of the line 1 to $n$ is odd,
    $\frac{n}{2}$ is not a whole number and does not correspond to the rank of
    any value in the data.  In that case, we have to work out what to do.
    There are various different methods to solve this [@hyndman1996sample], of
    which the most straightforward is to take the average of the values at the
    whole-number ranks either side of $\frac{n}{2}$.

## Unusual values compared to the distribution

So far, it looks as if AOC's district is fairly poor, and KOM's district is relatively wealthy.  But — are either of their districts *unusual* in their wealth or poverty?
...
plt.plot(mean_income, 2, 'ko', label='Mean')
plt.legend()
```

```{r, echo=FALSE}
hist(incomes, breaks=25,
    main='Median household income for 2023 congressional districts')
points(kom_income, 2, pch=16, col='red')
points(aoc_income, 2, pch=16, col='blue')
points(mean_income, 2, pch=16, col='black')
legend("topright",
       legend = c("KOM", "AOC", "Mean"),
