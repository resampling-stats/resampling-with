...
## Combining Boolean {{< var array >}}s and another take on the ships problem
...
We can do this by *combining* the Boolean {{< var array >}}s with the `&`
operator.  `&` combines Boolean {{< var array >}}s with a *logical and*.
*Logical and* is a rule for combining two Boolean values, where the rule is:
the result is {{< var true >}} if the first value is {{< var true >}} *and* the
second value if {{< var true >}}.
...
```{r montyblank, eval=TRUE, echo=FALSE}
...
```{r montyfull, eval=TRUE, echo=FALSE}
knitr::kable(fdf,
  booktabs = TRUE,
  row.names = TRUE,
...
Multiple clause for if (consider _and_).
...
shift_wins = np.repeat([False], 10000)
doors = [1, 2, 3]

for i in range(10000):
    my_door = rnd.choice([1, 2, 3])
    car_door = rnd.choice([1, 2, 3])
    # Which door will Monty open?
    montys_choices = []
    for door in [1, 2, 3]:
        # He can't open my door or the car door:
        if (door != my_door) & (door != car_door):
            montys_choices.append(door)
    # Choose Monty's door from the remaining options.
    if len(montys_choices) == 1:
    ...
    for door in [1, 2, 3]:
        if (door != my_door) & (door != montys_door):
            switch_door = door
    if my_door == car_door:
        stay_wins[i] = True
    if my_door == switch_door:
        switch_wins[i] = True
...
for (i in 1:10000) {
    ...
    for (door in doors) {
        # He can't open my door or the car door:
        if ((door != my_door) & (door != car_door)) {
            montys_choices = append(montys_choices, door)
        }
    # Choose Monty's door from the remaining options.
    if len(montys_choices) == 1:
        # There must be at least one remaining door.
