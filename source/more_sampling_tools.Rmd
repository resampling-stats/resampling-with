...
#
# 1. When opening the first trunk, it did not contain gold.
#    We will reject these trials, since they do not match our
#    experiment description.
# 2. Gold was found in the first and the second trunk.
# 3. Gold was found in the first, but silver in the second trunk.
#
# We need a placeholder value for all trials, and will make that
# "No gold in first trunk".

trial_outcome = np.repeat(['No gold in first trunk'], 10000)

for i in range(10000):
    # Select a ship at random from the three ships.
    ship_no = rnd.choice([0, 1, 2])
    trunks = ship_chests[ship_no]

    # We don't know which trunk is fore and aft,
    # so we shuffle them
    shuffled = rnd.permuted(trunks)

    if shuffled[0] == 'Gold':  # We found a gold trunk first.
        second_trunk = shuffled[1]

        # Store whether the second trunk was silver or gold.
        if second_trunk == 'Gold':
            trial_outcome[i] = 'Second trunk gold'
        else:
            trial_outcome[i] = 'Second trunk silver'

    # End loop, go back to beginning.

# Number of times we found gold in the second trunk.
n_golds = np.sum(trial_outcome == 'Second trunk gold')

# Number of times we found silver in the second trunk.
n_silvers = np.sum(trial_outcome == 'Second trunk silver')
...
parentheses — `ten_first_trunks == 'Gold' & ten_second_trunks == 'Gold'` to
...
The take away is, when using `&` to combine boolean arrays in Python,
put parentheses around the expressions on either side of `&`.
