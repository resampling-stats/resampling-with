...
[^list-of-lists]: Although we have used multiple `if` statements to set the
    ...
    # In this case, the elements in the list are vectors, on vector each ship.
...
[^ampersand-precedence]: We warned that we need parentheses around our `&`
    ...
    *operator precedence*; for example, the multiply operator `*` has *higher
    ...
    The point to take away is: when you are using `&` to combine Boolean arrays
    in Python, remember operator precedence, and make sure you have put
