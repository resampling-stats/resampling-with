...
Now let's show the effect of running a comparison on `first_10_trunks`:
...
```{python}
# The number of True values — so the number of "Gold" trunks.
np.sum(were_gold)
```
...
and `(ten_second_trunks == 'Gold')`.  In general, you will see that we will put
parentheses around these expressions that generate Boolean {{< var array >}}s,
and we recommend you do too, as good practice, and to make it clearer that
this is an expression that generates a value.

::: python
But — be careful — in this case, we actually *need* the parentheses to make
Python do what we want; see the footnote for more
detail.[^ampersand-precedence]
...
[^ampersand-precedence]: We warned that we need parentheses around our `&`
    ...
    `(ten_first_trunks == 'Gold') & (ten_second_trunks == 'Gold')`. Remember
    *operator precedence* the multiply operator `*` has *higher precedence*
    ...
    expression without parentheses — `ten_first_trunks == 'Gold' &
    ten_second_trunks == 'Gold'`, because of operator precedence, Python takes
    this to mean `ten_first_trunks == ('Gold' & ten_second_trunks) == 'Gold'`.
    Python does not know what to do with `'Gold' & ten_second_trunks` and
