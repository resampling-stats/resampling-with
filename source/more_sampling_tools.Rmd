...
## Selecting elements from {{< var array >}}s with slicing
...
colon as "up to, [but not]{.python}[and]{.r} including".  Finally, we have an
integer that gives the *stop* position. The slice, thus specified, asks {{< var
lang >}} to give us all the elements from (including) the *start* position, up
to, [but not]{.python}[and]{.r} including the *stop* position.

::: python
For example, here we index with a slice with *start* of 1 (offset 1 from the
...
stop value, the slice selects the elements starting at the *start* element, and
continuing through the last element of the array:
...
This means "get all the elements from position 3 to the end of the array".

Actually, we can even omit the start *and* the stop values, leaving just the
colon.  As you might expect, Python assumes 0 as the start, one past the end as
the stop, so the colon on its own means "return all the elements in the array".
As usual with slicing, we get a new array with the chosen elements.
...
::: r
For example, here we index with a slice with *start* position of 2 (the
position of the second element).  The *stop* is 5, meaning we should go up to,
and include, the element at position 5.  The result is another vector, that has
the elements of `some_numbers` from positions 2 through 5:
...
As promised, we now get the probability puzzles.
...
Boolean {{< var array >}}s are a fundamental tool in {{< var lang >}}, and we
will use them in nearly all our simulations.
