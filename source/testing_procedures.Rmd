...
for i in range(n):
    sample = rnd.choice([1, 2], size=10, replace=True)
    ones[i] = np.sum(sample == 1)

plt.hist(ones, bins='auto')
k = np.sum(ones >= 9)
print(k / n)
```
...
extended parallel discussion in @sec-confidence-two-approaches on confidence intervals for
