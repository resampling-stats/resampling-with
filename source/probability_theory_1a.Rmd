...
```{r fig-commanders-tree, echo=FALSE, fig.align='center', fig.cap="Tree diagram"}
...
can also be simulated with a computer. Using the {{< var lang >}} we produce
...
Notice that we are using the `print` function to show the values.  Previously
we have used the `message` function for this job.  `print` shows the values in
a slightly different way to `message`.  In particular, it displays the values
in a vector in a more readable and less compact form than `message`.  We will
be using both in this book, choosing one or the other depending on the value
we want to show.
:::
:::
...
```{r, opts.label="r_ed"}
# 0 = lose, 1 = win
cmdrs_win_when_no_rain <- numeric(100)
# Set the first 65 of 100 to have value 1.
cmdrs_win_when_no_rain[1:65] <- 1
# Show the result
cmdrs_win_when_no_rain
```

<!---
We need if blocks before here.
-->
...
for (i in 1:10000) {
    # draw one "ball" for the weather, store in "a"
    # 0 = rain, 1 = no rain
    a <- sample(weather, size=1)
    if (a == 1) {  # no rain
        # if no rain, check on game outcome
        # 0 = lose, 1 = win
        b <- sample(cmdrs_win_when_no_rain, size=1)
        if (b == 1) {  # Commanders win
            number_of_fine_wins <- number_of_fine_wins + 1
        }
    }
...
the Commanders will win — about `r round(get_var('prop'), 3) * 100`%.
...
probabilities that it will be a nasty day and the Commanders will lose, and
a nice day and the Commanders will lose. The sum of these probabilities should
come close to unity, because the sum includes all the possible outcomes. But
it will not *exactly* equal unity because of what we call "sampling variation"
or "sampling error."
...
<!---
The problem with writing code for this is that we need to introduce sampling with and without replacement.

In Python:

rnd.choice(cards)  # replace=True default
rnd.choice(cards, replace=False)

or, I guess, just use:

rnd.permute(cards)

In which case - we could introduce `permute` here, where we would still be using `sample` in the R case.

In R:

sample(cards, replace=TRUE)  # replace=FALSE default
sample(cards)
-->
