...
Notice that we have made the assumption here, that the probability of winning
the second game is the same as the probability of winning the first game. That
is, we are assuming the result of the first game does not influence the result
of the second in any way.  More technically, we are saying that result of the
second game is *independent of* (not influenced by) the result of the first.

Our procedure in this case — the prototype of all procedures in the resampling
simulation approach to probability and statistics — is as follows:
...
- *Theoretical Methods*.  The most fundamental theoretical approach is to
  ...
  when we use a tree diagram of the different outcomes and their
  probabilities.  We will come to tree diagrams in @sec-joint-probabilities.
  Tree diagrams allow us to calculate compound probabilities — for example
  — we could use a tree diagram to calculate the probability of three girls in
...
This book teaches you the empirical method using hypothetical cases. Formulas
can be misleading for most people in most situations, and should be used as
a shortcut only when a person understands exactly which first principles are
embodied in the formulas. But most of the time, students and practitioners
resort to the formulaic approach without understanding the first principles
that lie behind them — indeed, their own teachers often do not understand
these first principles — and therefore they have almost no way to verify that
the formula is right. Instead they use canned checklists of qualifying
conditions.
...
call a complete collection of all possible observations — a sample.)
...
1.  Probabilities are expressed as decimals from 0 through 1, like
...
R$ the *complement* of $R$, meaning, everything *other than* $R$.

We use "$\text{and}$" between two events to mean _both_ events occur.

For example, say we call the event "Commanders win the game": $W$.  One
example of a *compound event* (see above) would be the event $W \text{and} R$,
meaning, the event where the Commanders won the game _and_ it rained.  We
write the probability of this compound event as $P(W \text{and} R)$.
...
## Joint probabilities {#sec-joint-probabilities}
