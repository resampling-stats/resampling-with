...
With that, let us load and display the measured data for the two pig rations.

```{python}
# Use Pandas library to load data.
import pandas as pd

pig_rations = pd.read_csv('data/pig_rations.csv')
```

```{r}
pig_rations <- read.csv('data/pig_rations.csv')
```

```{r echo=FALSE, eval=TRUE}
._df_rations <- get_var('pig_rations')
ketable(._df_rations,
        caption = "Measured data for pig rations
        {#tbl-pig-rations}")
```
...
._weights <- ._df_rations$weight_gain
hist(._weights, breaks=20:40,
     main='Histogram of weight gains for both rations',
     xlab='Weight gain')
```
...
```{r label='fig-rations-ideal', fig.cap='Idealized distrubution for weight gain', echo=FALSE, eval=TRUE}
._x <- seq(20, 40, length=100)
._y <- dnorm(._x, mean=mean(._weights), sd=sd(._weights)) * sum(._weights)
plot(._x, ._y, type = "l",
     main='Idealized distribution of weight gains',
...
the two hands, and record it as in @tbl-pigs-measured. Repeat this procedure
many times.
...
```{r fig-liquor_price_plots, opts.label='svg_fig', fig.cap="Liquor prices by government and private"}
include_svg('diagrams/liquor_price_plots.svg')
```
...
291]. The data for the treatment and control groups are shown in Table 18-2.
