...
Let us first make some numbers from which to sample.  We will make the numbers
(range) 1 through 13 stand for the numerical values of the cards in one suit —
1 for ace, 2 through 10 for cards with 2 through 10 face value, 11 for Jack, 12
for queen, and 13 for king.
...
We have one suit, but we need a whole deck of cards, so four suits.  We can do
this by repeating `one_suit` four times:
...
We now have 52 card values, but the cards in the deck are in strict order.

We would like to select five of these "cards" (52 values) at random.  There are
two ways of doing this.  The first is to use the
[`sample`]{.r}'rnd.choice`]{.python}
tool in the familiar way, to choose 5 values at random from this strictly
ordered deck.  We want to draw these cards *without replacement* (of which more
later).  *Without replacement* means that once we have drawn a particular
value, we cannot draw that value a second time — just as you cannot get the
same card twice in a hand when the dealer deals you a hand of five cards.
...
Now let's do the same job using {{< var lang >}} on the computer. Let's name
"One Pair" the file which simulates a deck of playing cards and solves the
problem.
