...
for i in range(1, n_deals + 1):
    ...
    df.loc[i, 'Has pair?'] = 'Yes' if np.sum(counts == 2) == 1 else 'No'

pct = int(np.round(np.sum(df['Has pair?'] == 'Yes') / len(df) * 100))
n_cols = len(df.columns)
df.loc['', :] = [''] * n_cols
df.loc['**% Yes**', :] = [''] * (n_cols - 1) + [str(pct)]

cards_tab = df.to_markdown(tablefmt="grid")
print(cards_tab +
    '\n\n: Results of 25 hands for the problem "one pair" {#tbl-one-pair}')
```
...
n_one_pair = np.sum(pairs_per_trial == 1)
