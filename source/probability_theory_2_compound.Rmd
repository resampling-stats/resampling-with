...
* For each possible value (1 through 13), count the number of times each value
  has occurred in `hand`.  Call this calculation `number_of_repeats`.
* Select `number_of_repeats` values equal to 2; 
* Count the number of "2" values in `number_of_repeats`.  This the number of
  pairs, and excludes three of a kind or four a kind.
...
```{python, opts.label="py_ed"}
hand = np.array([5, 11, 5, 4, 11])
```

This hand represents a pair of 5s and a pair of Jacks (value 11).
...
```{python, opts.label="py_ed"}
number_of_1s = np.sum(hand == 1)
number_of_2s = np.sum(hand == 2)
number_of_3s = np.sum(hand == 3)
# And so on...
```

how many repeats there are of the numbers we got in `hand`.  Then we will look
for numbers that have only one repeat (a pair rather than three of a kind or
four of a kind).

Now we must find out if there is one (and only one) pair; we do this use the
{{< var bincount >}} function.
 statement — the "2" in that statement indicates that it is a
