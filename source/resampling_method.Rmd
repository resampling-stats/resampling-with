...
![10-sided die](images/one_d10s.png){#fig-one-d10s}
...
Random numbers are numbers where it is effectively impossible to
predict which number is coming next. If we ask the computer for a
number between 0 and 9, and we are unable to do better than chance in
...
We decide, arbitrarily, that if a die is rolled and lands on 9 that means the
...
```{python, opts.label="py_ed"}
# Count the number of nines among the random numbers, using the NumPy ("np")
# `count_nonzero` function.
b = np.count_nonzero(a == 9)
# Show the result
b
```
...
# The "sum" part counts how many numbers "a == 9" fod.
b <- sum(a == 9)
# Show the result
b
```
...
[`True`]{.python}[`TRUE`]{.r} or
[`False`]{.python}[`FALSE`]{.r}
values, depending on whether each element is equal to 9 or not.

Then, we ask {{< var lang >}} to
[`count_nonzero`]{.python}[`sum`]{.r}
only the true elements in that sequence.
...
The [`count_nonzero`]{.python}[`sum`]{.r}
...
The standard way to repeat steps in {{< var lang >}} is a `for` loop.  For example, let us say we wanted to display (`print`) "Hello" five times.  Here is how we would do that with a `for` loop:
...
The [`z[i] = b`]{.python}[`z[i] <- b`]{.r} statement that follows the
[`np.count_nonzero`]{.python}[`sum`]{.r}
...
[`np.count_nonzero`]{.python}[`sum`]{.r} at the end of the 1000 trials.  This
