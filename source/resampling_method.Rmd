...
# The result is a collection (array) of 20 numbers.
a
```
...
# Get 20 values from the *numbers* collection.
# Store the 20 numbers with with the name "a"
# We will explain the replace=TRUE later.
a <- sample(numbers, 20, replace=TRUE)

# The result is a collection of 21 numbers.
a
```
...
for i in range(5):
    # The indented stuff is the stuff we repeat five times.
...
# Read the next line as "repeat the following steps five times".
for (i in 1:5) {
    # The stuff between the curly brackets is the stuff we
    # repeat five times.
    # Print "Hello" to the screen.
    print("Hello")
}
...
To do this, we start with a collection of 1000 zeros, that we will fill in later, like this:
...
for i in range(1000):
    # The indented stuff is the stuff we repeat 1000 times.
