...
::: python
:::{#nte-numpy-scalar-display .callout-note}
...
the sum) came back from the Numpy library, because `np.sum` is a routine from
Numpy.  Numpy has a particular way of displaying single values that show us the
value (in this case `1`) and the *type* of the value (in this case `np.int64`).
The type of the value does not matter to us at the moment, but `np.int64` means
that this is a value of type *integer* (`int`) that uses `64` units
