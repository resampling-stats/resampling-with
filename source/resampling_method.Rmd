...
::: python
:::{.callout-note}
## Numpy display of single values

As you see above, Python showed us the value of the sum like this:
...
It is not important for our purposes, but in fact this value (the result of
the sum) came back from the Numpy library, because `np.sum` comes from Numpy.
Numpy has a particular way of displaying single values that show us the value
(in this case `1`) and the *type* of the value (in this case `np.int64`.  The
type of the value isn't important to us at the moment, but `np.int64` means
that this is a value of type *integer* (an integer is a positive or negative
whole number), and taking up 64 *bits* of the computer's memory.
...
```{python}
# Divide b by 2 and show the result.
b / 2
```
:::
:::
