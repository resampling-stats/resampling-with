...
Then, we ask `r book_lang` to `r py_or_r('count', 'sum')` only the
...
Now, it is our big moment! We get to combine the elements shown
above to perform our ambulance simulation over, say, 100 days.
...
multiple times (once for each day, in our case).  Here's how that
...
# The zeros are place holders that we replace with values later on.
# We'll call these our trial results.
z = np.zeros(100)

# The Python `range` command gives us the numbers 0..999.
# The for loop sets `i = 0`, runs the code,
#         then sets `i = 1`, runs the code again,
#         then sets `i = 2`, runs the code again, and so forth.

for i in range(100):
    # In other words, Python runs all the commands in this indented block
    # 100 times.
...
solved this problem using a ten-sided die. This is
...
still often unsure of the right answer. This is the grave disadvantage of
