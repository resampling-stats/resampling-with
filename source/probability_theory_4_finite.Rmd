...
PDF](https://resample.com/content/text/12-Chap-8.pdf).
...
The examples in @sec-infinite-universes dealt with *infinite universes* , in which the
...
```{r fig-success_case1, echo=FALSE, fig.align='center', fig.cap="The Event Classified as \"Success\" for Case 1"}
include_svg('images/success_case1.svg')
```
...
```{r fig-success_case2, echo=FALSE, fig.align='center', fig.cap="An Incomplete List of the Events Classified as \"Success\" for Case 2"}
include_svg('images/success_case2.svg')
```
...
```{r fig-success_case3, echo=FALSE, fig.align='center', fig.cap="An Incomplete List of the Events Classified as \"Success\" for Case 3"}
include_svg('images/success_case3.svg')
```

**Case 4.** What is the probability of getting two red balls and one
black ball in any order?

```{r fig-success_case4, echo=FALSE, fig.align='center', fig.cap="An Incomplete List of the Events Classified as \"Success\" for Case 4"}
include_svg('images/success_case4.svg')
```
...
```{r fig-success_case5, echo=FALSE, fig.align='center', fig.cap="An Incomplete List of the Events Classified as \"Success\" for Case 5"}
include_svg('images/success_case5.svg')
```
...
## Example 8-2: Five Spades and Four Clubs in a Bridge Hand (Multiple-Outcome Sampling Without Replacement, Order Does not Matter)
...
Let us assume that ace counts as 4, king = 3,
queen = 2, and jack = 1.
...
for i in range(N):
    ...
    hand = rnd.choice(whole_deck, 13, replace=False)
...
for (i in 1:N) {
    # Shuffle the deck of cards and draw 13
    hand <- sample(whole_deck, 13)  # replace=FALSE is default.
...
We see that in about `r get_var('kk')` percent of the trials did 4 or more
couples end up being re-paired with their own partners.  This can also be seen
from the histogram:
...
simulation-trial difference to the observed mean difference of \$4.84 - \$4.35
= \$.49; if it is as great or greater than \$.49, write "yes," otherwise "no."
...
problem and the treatment in @sec-infinite-universes is that the drawing in this case
is without replacement whereas in @sec-infinite-universes the drawing is with
replacement.
...
deck = np.repeat(['spade', 'others'], [13, 39])
...
# Deck with 13 spades and 39 other cards
deck <- rep(c('spade', 'others'), c(13, 39))
...
problems where we assume that the structure is known. Whereas @sec-infinite-universes
