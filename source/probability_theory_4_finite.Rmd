...
If they held a second round of the icebreaker, what is the chance that
four or more pairs will be the same?
...
for i in range(N):
    ...
    # (We could also have shuffled both arrays---it does not alter the outcome)
    ...
    matches = np.count_nonzero(shuffled_berkeley == birmingham)
...
**NOTE:** This seems to be 1.5 percent, instead of 3 now?
...
## Example 8-6: Matching Hats: Another famous problem of this sort: The hat-checker at a restaurant mixes up the hats of a party of 6. What is the probability that at least one will get his own hat?

Instead of matching pairs, as in the earlier problem, however, we are
now matching people with their hats. First, assign each of the 6
people a number, and place these numbers in A. Next, shuffle the array
(this represents the mixed-up hats) and compare to the original. The
rest of the problem is the same as the one before, except that we are
now interested in any trial where at least one (\>= 1) person received
the right hat.
...
managers = np.concatenate([
    np.zeros((10,)),  # worst managers
    np.ones((10,))  # best managers
])

for i in range(N):
    chosen = np.random.permutation(managers)[:10]
...
for i in range(N):
    ...
    hand = np.random.permutation(deck)[:13]
