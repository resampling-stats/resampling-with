...
weights = np.array([496, 544, 464, 416, 512, 560,
                    608, 544, 480, 466, 512, 496])
N = 10_000
bootstrap_means = np.zeros(N)
for i in range(N):
    bootstrap_means[i] = np.mean(rnd.choice(weights, size=len(weights)))
plt.hist(bootstrap_means, bins=25)
plt.title('Distribution of average weights from samples')
plt.xlabel('Average of weight sample');
```
...
(This problem would be approached conventionally with a binomial procedure
leading to quite wide confidence intervals [@deshpande1995statistical, p. 32]).
...
### Example: Confidence Interval for the Median Price Elasticity of Demand for Cigarettes
...
* **Step 2:** Draw a sample of 7 with replacement from the treatment group and
  calculate the mean.
* **Step 3:** Draw a sample of 9 with replacement from the control group and
  calculate the mean.
* **Step 4:** Calculate the difference in means (treatment minus control)
  & record.
* **Step 5:** Repeat steps 2-4 many times.
* **Step 6:** Review the distribution of resample means; the 5^th^ and 95^th^
...
Approach 1 that about 95 percent of the samples fall within .025 on either side
of the sample mean (which we had been implicitly assuming is the location of
the population mean). If we assume (and there seems no reason not to) that the
dispersions of the universes we experiment with are the same, we will find (by
symmetry) that the universe we seek is centered on those points .025 away from
.56, or .535 and .585.

From the standpoint of Approach 2, then, the conventional sample formula that
is centered at the mean can be considered a shortcut to estimating the boundary
distributions. We say that the boundary is at the point that centers
a distribution which has only a (say) 2.5 percent chance of producing the
observed sample; it is that distribution which is the subject of the
discussion, and not the distribution which is centered at $\mu = \bar{x}$.
Results of these simulations are shown in @fig-app-2-bush-duk.

![Approach 2 for Bush-Dukakis problem](images/25-Chap-21_004.png){#fig-app-2-bush-duk}
...
sample mean as midpoint will be used for guidance about where to locate trial
universes for Approach 2. The results for the tree diameter data
(@tbl-tree-diameters) are shown in @fig-app-2-trees.

![Approach 2 for tree diameters](images/25-Chap-21_005.png){#fig-app-2-trees}
