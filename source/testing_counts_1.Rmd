...
### Example: Does Irradiation Affect the Sex Ratio in Fruit Flies?
...
counts = np.zeros(n)
for i in range(n):
    flies = rnd.choice(['heads', 'tails'], size=14)
    counts[i] = np.sum(flies == 'heads')

strange = np.array(['No'] * n)
...
After one thousand samples have been drawn, we count (`sum`) how often there
