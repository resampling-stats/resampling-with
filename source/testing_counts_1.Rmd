...
we might flip a coin in groups of twenty flips, and count the number of heads
...
@tbl-fruitfly-trials shows the results obtained in twenty-five trials of twenty
flips each. In `r d2w[n_25_gte_14 + 1]` of the twenty-five trials
(`r round(n_25_gte_14 / n * 100)` percent) there were fourteen or more heads,
which we call "males," and in `r d2w[n_25_lte_6 + 1]` of the twenty-five trials
(`r round(n_25_lte_6 / n * 100)` percent) there six or fewer heads, meaning
there were fourteen or more tails ("females"). We can therefore estimate that,
even if the treatment does *not* affect the sex and the births over a long
period really are one to one, `r d2w[n_25_both + 1]` out of twenty-five times
(`r round(n_25_both / n * 100)` percent) we would get fourteen or more of one
sex or the other. Therefore, finding fourteen males out of twenty births is not
overwhelming evidence that the treatment has any effect, even though the result
is suggestive.

```{r, eval=TRUE, echo=FALSE}
counts <- flies[['# of heads']]
n <- length(counts)
...
the 100 trials `r d2w[n_gte_14 + 1]` contained fourteen or more "males"
...
`r d2w[n_25_gte_14 + 1]` of the twenty-five trials were fourteen or more males.
...
### Example: Does a treatment increase the female calf rate?
...
residents are for or against the awarding of a high-speed broadband internet
contract, and he asks you to take a poll. You judge that the voter registration
records are a fair representation of the universe in which the politician was
interested, and you therefore decided to interview a random selection of
registered voters. Of a sample of fifty people who expressed opinions, thirty
said "yes" they were for the plan and twenty said "no," they were against it.
How conclusively do the results show that the people in town want this internet
contract?
...
of "yeses" and "noes" in the first fifty draws, c) repeat for perhaps a hundred
...
chance. So our "significance level" or "prob value" is `r gte_30_pct` percent,
which is normally too high to feel confident that our poll results are
...
there would have been `r d2w[sum(yeses <= 20) + 1]`. This indicates how samples
can vary just by chance.

Taken together, the evidence suggests that the mayor would be wise not to place
very much confidence in the poll results, but rather ought to act with caution
or else take a larger sample of voters.
...
### Example: Comparison of Possible Cancer Cure to Placebo
...
In the trials shown in @tbl-pill-trials, in `r d2w[n_gte_3 + 1]` cases
...
### Example: Is One Pig Ration More Effective Than the Other?
...
`r d2w[n_rb_gte_9 + 1]` (marked by + signs) of the 25 (that is,
...
mean ranks between the two groups: (8/2 - 7/3 = 1.67).
...
6.  Calculate the difference between the means computed in step 5
    and record.
7.  Repeat steps 3 to 6 perhaps 1000 times.
...
for i in range(n):
    ...
    mean_differences[i] = np.mean(closer) - np.mean(further)

k = np.sum(mean_differences >= 1.67)
prob = k / n
...
for (i in 1:n) {
    ...
    mean_differences[i] <- mean(closer) - mean(further)
}

k <- sum(mean_differences >= 1.67)
prob <- k / n
...
a statistically significant difference in densities between the further planets
