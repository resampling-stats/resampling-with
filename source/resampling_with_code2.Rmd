...
Remember, we do *indexing* when we put square brackets following a value that
is a container, such as {{< var an_array >}}.  Inside the square brackets we
put another value to specify which elements we want to fetch from the
container.
...
colon as "up to, but not including".  Finally, we have an integer that gives
the *stop* position. The slice, thus specified asking Python to give us all the
elements from (including) the *start* position, up to, but not including the
*stop* position.
...
result is another array, that has the elements from positions 1 through 4 from
`some_numbers`:
