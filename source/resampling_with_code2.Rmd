...
We can now use {{< var np_or_r >}} and the random number functions from the
last chapter to get 12 random numbers from 0 through 99.
...
# Get 12 random numbers from 0 through 99
a = rnd.integers(zero_thru_99, 12)
# How many numbers are greater than 74?
b = np.sum(a > 74)
# Show the result
b
...
You could also break this description up into bullet points, to say the same
thing:

* For each item from your shopping basket:
...
of the checkout person above, the body of the for loop is:
...
The opening curly brace tells R that what follows is the procedure R should
follow for each item.  The procedure of the `for` loop is the lines of code
between the opening curly brace (`{`), and the closing curly brace (`}`).  The
lines *between the curly braces* are the *body of the for loop*.
:::

The initial line of the `for` loop about tells {{< var lang >}} that it should
...
the sequence in `author_birth_years`.  `birth_year` is called the
*loop variable*, because it is the variable that gets a new value each time we
go through the `for` loop procedure.  As for any variable in {{< var lang >}},
we can call our loop variable anything we like.  We used `birth_year` here, but
we could have used `y` or `year` or some other name.
...
`for` loop.  It can be a useful exercise to do this when you come across a
`for` loop, in order to work through the logic of the loop. For example, you
may want to write out the unrolled equivalent of the first couple of iterations,
to see what the loop variable will be, and what will happen in the body of the
loop.
...
Notice that we repeated the loop body 4 times.  We can also use the value from
...
```{python, opts.label="py_ed"}
for position in np.arange(0, 4):
    year = author_birth_year[position]
    print('Birth year of author position', position, 'is', year)
```
...
for (position in 1:4) {
    year <- author_birth_year[position]
    message('Birth year of author position', position, 'is', year)
}
```
...
position = 0
year = author_birth_year[position]  # Will be 1932
print('Birth year of author position', position, 'is', year)
position = 1
year = author_birth_year[position]  # Will be 1964
print('Birth year of author position', position, 'is', year)
# And so on.
```
...
trials, and that we could use `for` loops to do many repeats of steps like
those for a single trial.
...
juries.  These are the trials with a `z` value of 0.
