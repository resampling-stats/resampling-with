...
(were progression-free after two years).
...
Before we start, we again import NumPy and get its random number generator:
...
ten sided die, of which one side denotes "not cured":
...
`r begin_callout('Array data types')`
...
`r begin_callout('For-loops in Python')`
We first saw for-loops in Chapter \@ref(resampling-method).  We will now dive into them a bit more deeply.
