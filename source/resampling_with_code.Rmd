...
We know that the average cure rate in the developed world is 90%.  Lacking any
further information, we could therefore think of each patient being treated as
having a 90% chance of being cured.  If a patient now receives the new
treatment and is cured, does this tell us whether the new treatment is better
than the standard treatment? We don't really know: as long as the new
treatment is not a lot worse than the standard treatment, curing the patient
is the outcome we expect.
...
We can use the computer to do something very similar, by asking the computer
for 17 random whole numbers from 0 through 9.
...
us also say that we know for sure that 50% of the time we get a head and 50%
we get a tail.  We can predict some things about this coin toss.  We can
predict that it will either be a head or a tail, and we can predict that it
will be a head 50% of the time. But we can't predict *which one* of heads or
tails we will get on the next throw.  We say the result of each throw is
...
practical sense — meaning that there is no *practical* way for you or anyone
else to predict the next number.  These practical-sense random numbers are
also called *pseudo-random* numbers.  The pseudo-random numbers from
[NumPy]{.python}[R]{.r} use some very serious machinery to make the numbers
hard to predict, and practically random.  No-one can guarantee that it is
absolutely *impossible* to find some way of improving our prediction for the
next pseudo-random number that
[NumPy]{.python}[R]{.r}
gives us, but we, and the rest of the scientific world, do not know of any way
to do it. That practical sense of randomness from the computer is good enough
for every case that we cover in this book, and we bet it will also be true for
every case that you will come across in practice.
:::
