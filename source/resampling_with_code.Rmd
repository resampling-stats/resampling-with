...
For example, consider the [R]{.r}[Numpy]{.python} *function* `round`.

::: python
:::

```{python, opts.label="py_ed"}
# We fetch the `round` function from the Numpy library (module).
from numpy import round
```

`round` is the name for a simple production line, that takes in a number, and
sends back the number rounded to the nearest *integer*.
...
Therefore, think of `round` as the name of a production line, that takes in a
*component* (in this case, any number), and does some work, and sends back the
finished *result* (in this case, the number rounded to the nearest integer.
...
See @fig-round_function_pl for an illustration of `round` as a production
line.
...
Like many functions, `round` can take more than one argument (component).  You
can also send `range` the number of digits you want to round to, like this:
...
syntax]{.r}[standard Numpy function `arange`]{.python} to do that.
...
::: python
Notice that we send `arange` the *arguments* 0 and 10.  Think of the first
argument, here 0, as the *start value*.  The second argument, here 10, is the
*stop* value.  Numpy (in the `arange` function) understands this to mean: *start* at 0 (the start value) and go up to *but do not include* 10 (the stop value).

Here we have sent `arange` two arguments, but we can also send just one argument, like this:
...
Here are some more examples of [the colon syntax]{.r}[`arange`]{.python}:
...
```{python, opts.label="py_ed"}
# Here we are only sending one value (7).  arange understands this to be
...
```{r, opts.label="r_ed"}
# Select an integer from the choices in some_integers.
my_integer <- sample(some_integers)
# Show the value that results.
my_integer
```

Like `range` (above), [`rnd.choice`]{.python}[`sample`]{.r} is a *function*.
...
We sent
[`rnd.choice`]{.python}[the `sample` function]{.r}.
a value to work on, in this case, the value of `some_integers`.  The
values we send to functions are called function function *arguments*.

[@fig-rnd_choice_pl]{.python}[@fig-sample-pl]{.r} is a diagram illustrating an
...
# some_integers is the *argument*.
# Put the *return* value from the function into "my_number".
my_number <- sample(some_integers)
# Show the value that results.
my_number
```
...
In the code above, we asked {{< var sample >}} to select a single number at
random.

In fact, the people who wrote {{< var sample >}}, wrote it to be flexible in
the work that it can do.  In particular, we can tell
{{< var sample >}} to select *any number of values* at random, by adding a new
...
`replace=TRUE` tells R to sample `some_integers` *with replacement*.  We will
