...
4. We *repeat* the simulated trial procedure many times, and collect the
   ...
   will this a *run* of 100 trials.
5. We *count* the number of trials with an outcome that matches the outcome we
   are interested in. In this case we are interested in the outcome 17 out of 17 cured, so we count the number of trials with a score of 17.  Say 15 out of the run of 100 trials had an outcome of 17 cured. That is our *count*.
...
For now, think of a function as a named *production line*.  We sent the
function raw material to work on.  The production line does some work on the
raw materials.  A finished result comes off the other end.

In our case, the production line has the name [`rnd.choice`]{.python}[the
`sample` function]{.r}.
...
`round` is another function that we may find ourselves using.
`round` is the name for a very simple production line, that takes in a number,
and sends back the number rounded to the nearest *integer*.

:::{.callout-note}
## Integers

Therefore, a number is an *integer* if the number is *either* a whole number
(0, 1, 2 ...), *or* a negative whole number (-1, -2, -3 ...). All of -208, -2,
0, 10, 105 are integers, but $\frac{3}{5}$, -10.3 and 0.2 are not.
...
For example, here you see `round` taking in a not-integer number, as an *argument*, and sending back the nearest integer, as the result:
