...
```{r fig-round_function_pl, opts.label='svg_fig', fig.cap="The `np.round` function as a production line"}
...
Way back in @nte-numpy-scalar-display, you saw that Numpy has a particular way
...
```{python}
# Display result of Numpy sum of 1 and 2.
np.sum([1, 2])
```

As Notice the output gives the value (


You will see many other Numpy values displayed in this form.  For example,
this is how Numpy displays a *floating point* value (a value that need not be
a whole number):

```{python}
# Divide b by 4 and show the result.
b / 4
```
:::
:::
