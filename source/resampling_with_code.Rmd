...
```{r fig-round_function_pl, echo=FALSE, fig.align='center', fig.cap="The `round` function as a production line"}
include_svg('images/round_function_pl.svg')
```
...
```{r fig-round_ndigits_pl, echo=FALSE, fig.align='center', fig.cap="`round` with optional arguments specifying number of digits"}
include_svg('images/round_function_ndigits_pl.svg')
```
...
```{r fig-round_function_named, echo=FALSE, fig.align='center', fig.cap="The `round` function with argument names"}
include_svg('images/round_function_named.svg')
```

```{r fig-np_round_function_named, echo=FALSE, fig.align='center', fig.cap="The `np.round` function with argument names"}
include_svg('images/np_round_function_named.svg')
```
...
(library).  The `a` in `arange` refers to the fact that the result that comes back from `np.arange` is an *array*:
...
format, but Python has another way of getting a range of integers — by using
the Python `range` function.

The `range` function is very similar to `np.arange`, but it is *not* part of
Numpy, and it does *not* return an *array* of numbers.  Here we ask for a `range` from 0 through 6 (0 up to, but not including 7):
...
in for* the number 0 through 6.  It is not an array, but specific type of
...
number out of the container in many different ways, but one of them is to
convert this container to an array, using the `np.array` function.  The
`np.array` function takes the thing we pass it, and tries to make it into an
array.  When we apply `np.array` to `r` above, we get the numbers that `r`
contains:
...
Another way we can get the numbers from the `range` thing, is to cycle through the numbers one by one, using a `for` loop.  For example, we could ask the `for` loop to fetch each of the numbers in `r`, one by one, and then do something to them.  In this case we just print out the numbers.

```{python, opts.label="py_ed"}
# Take each of the numbers represented in r, one by one ...
for value in r:
    print('This number', value)
```

Of course, we could do the same `for` loop, but constructing the `range` value on the fly, like this:

```{python, opts.label="py_ed"}
# Take each of the numbers represented in range(7), one by one ...
...
in the range 0 up to (not including) 100000 into the variable `value`, and
then do something in the body of the loop.  But just to be clear, we could just as easily have written the cell above as:

```{python, opts.label="py_ed"}
# Take each of the numbers in the array, one by one ...
for value in np.arange(7):
    print('This number', value)
```

But — even though we could use `np.arange` to get an array of numbers, we
generally prefer `range` in our Python `for` loops, because it's a more common
pattern in Python.[^range-efficiency]
...
[^range-efficiency]: Actually, there is a reason why many Python programmers
   ...
   efficient container, in that it doesn't need to have all the memory needed
   to make the full array, it just needs to know what number to give you next.
   For example, consider `for i in np.arange(10000000):` — in this case Python
   has to make an array with 10,000,000 elements, and then, from that array,
   it passes each value one by one to the for loop. `for i in
   range(10000000):` will do the job just as well, passing the same sequence
   of 0 through 9,999,999 to `i`, one by one, but `range(10000000)` never has
   to make the whole 10,000,000 element array — it just needs to keep track of
   which number to give up next.  Therefore `range` is very quick, and very
   efficient in memory.  This doesn't have any great practical impact for the
   arrays we are using here, typically of 10,0000 elements or so, but it is
   worthwhile for larger arrays.

Notice that `range` has the same `start` and `stop` arguments that `np.arange`
...
::: python
```{r fig-rnd_choice_pl, echo=FALSE, fig.align='center', fig.cap="Example run of the `rnd.choice` function"}
include_svg('images/rnd_choice_pl.svg')
```
:::

::: r
```{r fig-sample_pl, echo=FALSE, fig.align='center', fig.cap="Example run of the `sample` function"}
include_svg('images/sample_pl.svg')
```
:::
...
::: python
In the code above, we asked Python to select a single number at
random — because that is what `rnd.choice` does by default].

In fact, the people who wrote `rnd.choice`, wrote it to be flexible in
the work that it can do.  In particular, we can tell
`rnd.choice` to select *any number of values* at random, by adding a new
...
the basis for one simulated trial in the world of Saint Hypothetical General.
