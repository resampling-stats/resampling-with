...
question — *how surprised are we that the NCI researchers saw their result of 17 out of 17 patients cured?*
...
{{< var np_or_r >}} also allows *values* that are *sequences of numbers*.  {{< var np_or_r >}} calls these sequences *{{< var array_name >}}s*.

::: r
The name *vector* sounds rather technical and mathematical, but the key idea is only that a *vector* stores a sequence of numbers.
:::
...
we will be using them all the time.
...
sequence of `some_numbers`.  As you might have guessed, {{< var np_or_r >}}
will give us these numbers as a[n]{.python} {{< var array_name >}}:
