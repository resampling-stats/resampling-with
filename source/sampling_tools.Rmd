...
```{r, opts.label="r_ed"}
# Get 1 random numbers from 0 through 99
...
After that, we have to unpack our labeling of 0 through 25 as being "black" and
26 through 99 as being "white".  We might do that like this:
...
juror_types = np.array(['black', 'white'])
# The number of times we want to repeat "black" and "white".
repeat_nos = np.array([26, 74])
...
n_black = np.sum(rnd.choice(jury_pool, 12) == 'black')
n_black
```

```{r, opts.label="r_ed"}
# The same as above, but on one line.
n_black = sum(sample(jury_pool, 12, replace=TRUE) == 'black')
n_black
```
...
We looked at the composition of Robert Swain's jury — but in fact, by law, that
does not have to be representative of the eligible jurors.   The group that
does have to be representative of the eligible population, by law, is the
larger group of 100 jurors from which the 12-person jury was selected.  In fact
the 12 person jury is also called the [petit
jury](https://en.wikipedia.org/wiki/Petit_jury) from the French *petit*,
meaning small.  Part of the trial procedure is *jury selection*, that involves
selecting the 12-person *petit jury* from the much larger *jury panel*, that,
for Robert Swain, was 100 people, of whom only 8 were black.

The jury panel is required, by law, to be a representative sample of the
eligible jurors.

We already know that, of the eligible jurors, 26% were black.  We now know that
8 out of 100 (8%) of the jury panel were black.  We will leave it to you to
adapt the simulation from @sec-resampling-two to ask the question — is 8%
surprising as a random sample from a population with 26% black people?

But we have a different question: given that 8 out of 100 of the jury panel
were black, is it surprising that none of the 12 jurors in the petit jury were
black?

Let's think about what a single simulated jury selection would look like.

First we compile our jury panel, using the tools we have used above.  See the code above for more explanation about the individual lines.

```{python, opts.label="py_ed"}
juror_types = np.array(['black', 'white'])
# in fact there were 8 black jurors and 92 white jurors.
panel_nos = np.array([8, 92])
...
strange_jury = rnd.choice(jury_pool, 12)
strange_jury
```

```{r, opts.label="r_ed"}
# Take a sample of 12 jurors from the panel *with replacement*
strange_jury <- sample(jury_pool, 12, replace=TRUE)
strange_jury
```
...
ok_jury = rnd.choice(jury_pool, 12, replace=False)
ok_jury
```
...
```{r, opts.label="r_ed"}
# Take a sample of 12 jurors from the panel *with replacement*
ok_jury <- sample(jury_pool, 12)  # replace=FALSE is the default.
ok_jury
```
<!---
End of notebook
