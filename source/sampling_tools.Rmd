...
The {{< var lang >}} [`str`]{.python}[`character`]{.r} is a bit of text, and
therefore consists of a sequence of characters.
...
```{r, opts.label="r_ed"}
# Julian Lincoln Simon's second name
middle_name <- arr_of_strings[2]
middle_name
```

As for numbers, we can compare strings with, for example, the `==` operator, that asks the question whether the two strings are equal:
...
result is easier to see, because we are using the string labels, instead of
numbers:
