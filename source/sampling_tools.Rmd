...
strange_jury = rnd.choice(jury_panel, 12)
strange_jury
```

```{r opts.label="r_ed"}
# Take a sample of 12 jurors from the panel *with replacement*
strange_jury <- sample(jury_panel, 12, replace=TRUE)
strange_jury
```
...
```{r opts.label="r_ed"}
# Take a sample of 12 jurors from the panel *with replacement*
ok_jury <- sample(jury_panel, 12)  # replace=FALSE is the default.
ok_jury
```
...
for i in np.arange(10000):
    ...
    jury = rnd.choice(jury_panel, 12, replace=False)
