...
is smaller.[^mean-product], so the products will be smaller, and the mean
...
line must be the same. The slope of the line is a measure of how much the IQ
score increases as the athletic score increases — a good measure of the linear
association between athletic score and IQ score. Because the slope has not
changed between the original IQ score and the IQ-100 score, the association has
not changed, even though the mean product is much smaller.
...
out = pd.DataFrame(
    data={'Original IQ': _iq,
          'IQ deviations': np.round(_iq - np.mean(_iq), 2),
          'Standard scores': np.round(_std_iq)
          }
)
...
product-moment correlation coefficient*, often referred to simply as the
*correlation coefficient*.  We usually use the letter $r$ to refer to the
...
give a standard measure of association (@tbl-r-calculation).

```{python eval=TRUE, echo=FALSE, results="asis", message=FALSE}
_std_ath = _standard_scores(_ath)
_std_comp = _standard_scores(_compliance)
_std_chol = _standard_scores(_cholesterol)
out = pd.DataFrame(
    data={'Athletic score': _ath,
          'Original IQ': _iq,
          ...
          'Cholesterol': _iq,
          'Standardized\ncompliance': _std_comp,
          'Standardized\ncholesterol': _std_chol,
          'Col 8 x Col 9': _std_comp * _std_chol,
          }
)
...
suff.iloc[:, 0] = ['', '**r value**']
out2 = pd.concat([out, suff])
r_tab = footerize_table(
    to_md(out2, np.arange(1, out2.shape[1] + 1)),
    [-2, -1])

print_tab(r_tab,
          'r values from means of standardized products',
          'r-calculation}')
```
