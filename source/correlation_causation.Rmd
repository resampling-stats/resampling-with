...
_s = 1.6
_c = np.mean(_iq) - _s * np.mean(_ath)
_pos_iq = np.round(_c + _s * _ath + rnd.normal(0, 5, size=len(_iq)))
...
_s = -1.6
_c = np.mean(_iq) - _s * np.mean(_ath)
_neg_iq = np.rround(_c + _s * _ath + rnd.normal(0, 5, size=len(_iq)))
...
```{python label="fig-given_iq_athletic", fig.cap="Given Scores for I.Q. and Athletic Ability", eval=TRUE, echo=FALSE}
aiq_plot(_ath, _iq)
```
...
graph to continue the analysis. The next step is to investigate whether
a measure of relationship is statistically significant; this we do as follows:
...
To accomplish this aim, we repeat the following procedure 10000 times using
