...
### Example: sum of products correlation of athletic and IQ scores
...
samp_sums = samp_out.agg(lambda col : np.sum(col * _ath), axis='index')
...
compliance = np.array(cholost_df['percent_compliance'])
cholesterol = np.array(cholost_df['cholesterol_decrease'])
...
compliance <- cholost_df$percent_compliance
cholesterol <- cholost_df$cholesterol_decrease
...
We have been using the *product of sums* as a measure of straight-line
(linear) association between two variables.   We found that the sum of products
for the athletic and IQ variables (`ath` and `iq`) was `r not_e(sop_aiq)`, and
we showed that the sum of products is higher for pairings of `ath` and `iq`
with a higher degree of linear association.  Now let us consider compliance and
...
is much higher than for `ath` and `i` — but that does not tell us whether the
association of the scores is higher.  There are various problems that prevent
us doing a direct comparison between the two scores.  We would like a version
of the sum of products procedure that could give us a measure of linear
association that we could compare between different pairs of sequences.   This
is the purpose of the *correlation coefficient*.
...
to get the *mean* of the products.  The mean of product values for the athletic / IQ scores is `r not_e(sop_aiq)` / `r length(._ath)` =
...
population.  These have a mean of `r round(mean(._iq), 2)`.  But we could also
have scored IQ in another way.  For example, we could have recorded the
*difference* from 100 instead of the raw value, to get the values in column
2 of @tbl-iq-minus-hundred:
...
Notice that the mean product of the IQ-100 is much smaller than the mean for the product 
...
```{python eval=TRUE, echo=FALSE, results="asis", message=FALSE}
_deviations = _ath - np.mean(_ath)
out = pd.DataFrame(
    data={'Athletic score': _ath,
          'IQ score': _iq,
          'Col 1 x Col 2': _ath * _iq,
          'Athletic deviations': _deviations,
          'Col 2 x Col 4': _deviations * _iq}
)
...
sum of products to compare the association between two different
...
_deviations = _ath - np.mean(_ath)
out = pd.DataFrame(
    data={'Original athletic score': _ath,
          'Actual IQ': _iq,
          'Col 1 x Col 2': _ath * _iq,
          'Athletic deviations': _deviations,
          'Col 2 x Col 4': _deviations * _iq}
...
```{python eval=TRUE, echo=FALSE, results="asis", message=FALSE}
_ath_ds = _ath - np.mean(_ath)
_iq_ds = _iq - np.mean(_iq)
out = pd.DataFrame(
    data={'Original athletic score': _ath,
          'Original IQ': _iq,
          'Athletic deviations': _ath_ds,
          'IQ deviations': np.round(_iq_ds, 2),
          'Col 3 x Col 4': _ath_ds * _iq_ds}
)
