...
a "correlation coefficient" close to zero indicates little or no linear
relationship between two variables; correlation coefficients close to 1 and -1
denote a strong positive or negative relationship. We will generally use
a simpler measure of correlation than the correlation coefficient, however.
...
## Some terminology, with a recap on sum of products

Thus far we have been using terms such as *association* and *correlation* rather loosely.  It is time to define terms more formally.

We are interested in the situation where we have two *sequences* of values. For
example, the Athletic scores are one sequence of values, and the IQ scores are
another.   These two sequences have to be the same length (the same number of
elements).   We will often use the name $n$ to refer to the number of values in
the sequences.  For example, for the compliance / cholesterol data, there are
`r length(get_var('x'))` values so, in that case, $n =$ `r
length(get_var('x'))`.  We often name these sequences in text and in the code
— for example we've used `ath` for the sequence of athletic scores and `iq` as
a name for the sequence of IQ scores.  We can also refer to these named
sequences (such as `ath` or `iq`) as *variables*; a variable is a named thing,
and in the case of `ath` and `iq` , the named thing is a sequence of values.

We are thinking here about the *association* of two sequences; that is,
broadly, the tendency for the values in one sequence to vary in some
predictable way in relation to another.   In particular, we are interested in
*linear association*; that is the tendency for one sequence, such as `iq` to
vary in a way that is predictable from a straight line based on the other, such
as `ath`.  When there is linear association between two sequences, such as
`ath` and `iq`, we can make a better than random estimate of the values we want
to predict (e.g. the `iq` values), with a straight line based only on we are
predicting with (e.g. `ath`) values.   Call the values we are trying to predict
— the target sequence or variable, and the values we are trying to predict with
— the predictor sequence or variable.  For example, above, `iq` is the target
variable (or sequence) and `ath` is the predictor variable (or sequence).
Likewise cholesterol sequence (named `y` in the code above) is our target
variable, and the compliance scores (`x` in the code) is our predictor
sequence.

For now, let's stick with the variable names `x` (the predictor) and `y` (the
target). We have found that we can check the strength of the linear association
between two variables (sequences), such as `x` and `y`, by using the sum of
products.  We get the sum of products by first lining up the two length $n$
sequences `x` and `y` next to each other, and forming pairs of values (first
`x` value with first `y` value, second `x` value with second `y` value and so
on), giving us $n$ pairs.  We multiply the elements of each pair together to
get the product, and take the sum of all $n$ products, across pairs.  We have
seen that this sum will be greater when the `x` and `y` values have a closer
linear association, meaning, when the `x` values do a good straight-line
prediction of the `y` values.  We can use this sum-of-product calculation to
compare different pairings of the `x` and `y` values — for example, if we
shuffle, say, the `x` values, and line the shuffled values `x` values up with
the `y` values, and calculate the sum of products for this new set of pairs, it
will have a higher sum of products if the new set of pairs does a better job of
linear prediction of `y` than the original pairing, and lower if it does
a worse job.
...
We have been using the *product of sums* as a measure of linear association
between two variables.   For example, we found that the sum of products for the
athletic and IQ variables (`ath` and `iq`) was `r sop_aiq`.   We can use the
sum of products to compare the linear association between two different
pairings *of the same variables*.  For example, in @tbl-ath-iq-products, we
calculated sum of products for different ordering of the `ath` variable, and
found that the original ordering of `ath` gives a sum of products that is lower
than the pairing of `ath` sorted to match `iq`, but higher than the pairing of
`ath` sorted to match the reverse ordering of `iq`.  That is, the actual `ath`,
`iq` pairing is intermediate in linear association, between the best and worst
possible linear association, as measured by the sum of products.

The sum of products is useful because it allows us to compare the strength of relationship between different pairings — if the sum of products is higher for another ordering of the data, it means that the other ordering corresponds to a better match in order between the values in the pairs.

For example, we found that sum of products for the sorted athletic and sorted
IQ scores, where the match is as good as it can be, has a higher sum of
products.  In fact the value is `r not_e(sop_aiq)` (see @tbl-ath-iq-products).

So, the sum of products is useful to compare the strength of relationships
*between orderings* of the same values, but it does not give us a useful idea
of the strength of the relationship compared to other sequences of values.

For example, the sum of products for the compliance and cholesterol values was
`r not_e(sop_cc)`.  That is a much higher value, but we cannot conclude that the relationship between the respective sequences was stronger, because there are various features of the values that will affect the sum of products.

We would like a measure of association that we can compare between different sets of values.  That is the role of the *Pearson product-moment correlation coefficient* or $r$.  As you will see, it also involves the product of sums, but after adjusting for various factors that are not relevant to the strength of association.

The first factor we would like to remove is the scale we have used for our
measures.  For example, our athletic scores vary from `r min(._ath)` to
`r max(._ath)`, but we could instead have decided to subtract the mean from
each score to give the *deviations*.  The deviations tell us how far each
person was above or below the mean of the group.  The mean of the athletic
scores is `r mean(._ath)`. @tbl-ath-deviations has the deviations and the
corresponding sum of products.

```{python eval=TRUE, echo=FALSE, results="asis", message=FALSE}
deviations = ath - np.mean(ath)
out = pd.DataFrame(
    data={'Original athletic score': ath,
          'Actual IQ': iq,
          'Col 1 x Col 2': ath * iq,
          'Athletic deviations': deviations,
          'Col 2 x Col 4': deviations * iq}
)
...
sums.iloc[0] = '**SUMS**'
out2 = pd.concat([out, sums.to_frame().T])
dev_tab = to_md(out2, np.arange(1, out2.shape[1] + 1))

print_tab(dev_tab,
          'Products for original values and deviations',
          'ath-deviations')
```

Notice that the relative *ordering* of the deviation scores, relative to the IQ
scores, is the same as for the original scores — the lowest of the original
scores also has the lowest deviation, and so on.  We would like our ideal
measure of association to reflect that the deviations and original values have
the same relative ordering, and should therefore have the same measure of
association, but the simple sum of product measure does not.  The deviation sum
of products with IQ is much lower than the sum of products of the original
scores with IQ.

:::{.callout-note}
## Adding or subtracting values and the sum of products

Here we explain the change in the sum of products after subtracting the mean with a little algebra.  Skip this note if you like; you could come back to it later when you have finished this section or page.

Why is the sum of products for the deviations smaller than the sum of products for the original scores?

This is because we can think of each of the original scores as being made up of
the deviation plus the mean (`r mean(._ath)`.  For example the first Athletic
score is `r ._ath[1]`.  Write this first value as $a_1$.  Write the mean of all
the athletic scores as $m$ ($m$ = `r mean(._ath)`). The first deviation $d_1$
is given by $d_1 = a_1 - m$ = `r ._ath[1] - mean(._ath)`.  Call the first IQ
value $q_1$ ($q_1$ = `r ._iq[1]`).  Therefore the first product in the original
sum of products is $a_1 * q_1$ which we can also write as $(d_1 + m) * q_1
= d_1 * q_1 + m * q_1$.   Write the second athletic score as $a_2$ and the
second IQ score as $q_2$, and so on.  We defined the original sum of products
as $a_1 * q_1 + a_2 * q_2 + a_3 * q_3 ... + a_{10} * q_{10}$, so we can also
write this as $(d_1 + m) * q_1 + (d_2 + m) * q_2 + (d_3 + m) * q_3 ...
+ (d_{10} + m) * q_{10}$ = $(d_1 * q_1 + m * q_1 + d_2 * q_2 + m * q_2 + d_3
* q_3 + m * q_3 ... + d_{10} * q_{10} + m * q_{10}$.   Because we can re-order
  our multiplications as we please (multiplication is *associative* — $a
* b = b * a$), we can also get the same result with adding the deviation sum of
products $(d_1 * q_1 + d_2 * q_2 + d_3 * q_3  ... + d_{10} * q_{10}$) to the
sum of the IQ values times the mean ($m * q_1 + m * q_2 + m * q_3 + ...
+ m * q_{10}$).  Let's do that with the actual numbers:

```{python}
# Rename the sequences to match the text.
a = ath
q = iq
m = np.mean(ath)
# The deviations.
d = a - m
deviation_sum_of_products = np.sum(d * q)
deviation_sum_of_products
```

```{python}
# Sum of the mean times the IQ values.
mean_q_sum = np.sum(m * q)
mean_q_sum
```

```{python}
original_sum_of_products = np.sum(a * q)
original_sum_of_products
```

```{python}
# We can also write the same calculation as:
deviation_sum_of_products + mean_q_sum
```
...
# The deviations.
d <- a - m
deviation_sum_of_products <- sum(d * q)
deviation_sum_of_products
```
...
```{r}
original_sum_of_products <- sum(a * q)
original_sum_of_products
```
...
Notice the `mean_q_sum` doesn't depend on the ordering if IQ — the mean $m$ is
always the same, so if you reorder IQ (`q`), you'll always get the same result
for `mean_q_sum`.  So we can always get the sum of products for a set of deviations, by subtracting the `mean_q_sum` value from the corresponding sum of products on the original values.
:::

In pursuit of finding a measure that doesn't depend on the average of the individual sequences, let us start by replacing the original athletic and IQ scores with their deviations from their respective means, and recalculate the sum of product score.  The mean of the IQ scores is `r mean(._iq)`.  The result is @tbl-ath-iq-deviations:

```{python eval=TRUE, echo=FALSE, results="asis", message=FALSE}
ath_ds = ath - np.mean(ath)
iq_ds = iq - np.mean(iq)
out = pd.DataFrame(
    data={'Original athletic score': ath,
          'Original IQ': iq,
          'Athletic deviations': ath_ds,
          'IQ deviations': np.round(iq_ds, 2),
          'Col 3 x Col 4': ath_ds * iq_ds}
)
...
sums.iloc[0] = '**SUMS**'
out2 = pd.concat([out, sums.to_frame().T])
dev_both_tab = to_md(out2, np.arange(1, out2.shape[1] + 1))

print_tab(dev_both_tab,
      'Products for original values and deviations for both scores',
      'ath-iq-deviations')
```
