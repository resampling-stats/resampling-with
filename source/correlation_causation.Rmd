...
`r length(get_var('x'))` values so $n =$`r length(get_var('x'))`. The sequences
have to be the same length because we are looking at the *pairs* formed by
taking the two sequences, and looking at corresponding 
...
Thus far we have been using the *product of sums* as a measure of the
relationship between pairs of values.

We found that the sum of products for the athletic and IQ scores was `r sop_aiq`
