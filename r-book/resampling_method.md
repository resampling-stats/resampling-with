...
<p>
<div class="nb-links">
<a class="notebook-link" href=ambulances.Rmd>Download notebook</a>
...
*comments*.  When R sees the `#` at the start of the line, it
realizes that it should ignore everything else on that line, and skips on the
next line. Here's an example of a comment in R:
...
Because R ignores lines beginning with `#`, the text comments is
...
Our next task is to use R to simulate a single day of ambulances.
...
random drawing carried out by the `sample` operation.
The result of the `sum` operation will be
...
Now let us place the  `sample`and `r
...
(`sum`) at the end of the 1000 trials.  This
...
simulate the analogy on the computer using some R code like the code
above.
