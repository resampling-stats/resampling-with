...
why, and say a little about where this book fits into your learning and
teaching about data analysis and statistics.
...
statistical recipes for common situations. It applies warning labels to the
recipes, in the hope that students will later use the right statistical tests,
even if they have little understanding of what those tests mean.  Of course,
the cookbook method is a miserable way to learn, and a miserable way to teach.
Our better students see that we are not teaching for understanding, but
rehearsal, and they suffer the most.  It may be no comfort, but we teachers
also suffer when we have to teach this way.
...
reasonably teach to most of our students [@cobb2007introductory].  As Wallis
and Roberts [-@wallis1957statistics] point out in their textbook, the large
majority of our students are not fluent in mathematics, and teaching
statistics by way of mathematics is like teaching philosophy in a foreign
language.

Simon went two steps further than this, although, as you will see, those two
steps can also be one giant leap.  His first step was the insight that
mathematics was not the only thing that made statistics difficult. He saw that
the big, underlying problem is that statistical inference --- drawing
conclusions from noisy data --- involves a lot of hard, clear thinking about
the data, and the process that made the data.  This hard thinking remains,
even if you can strip nearly out the mathematics (you can!).   He writes
eloquently about this in the introduction of this book, in a section "Why is
statistics such a difficult subject".   Because statistics is inherently
difficult, it is all the more important to find ways of avoiding the extra
difficultly that comes from teaching through mathematics.  This inherent
difficulty of statistics can be much more interesting to students of data than
the first, incidental and often unnecessary difficulty of understanding data
through mathematics.  The inherent difficulty is just that of thinking clearly
about the problem, and designing a solution.  It is the difficulty of thinking
logically about data, and noise.
...
powerful and general languages, such as Python and R, that are now
...
the center of the modern movement of *data science*.  We discuss the meaning
of data science below, but for now, let us say it is some fusion of statistics
and computing.
...
describe and understand.  A later discussion paper and responses made similar
points [@cobb2015mere, @horton2015mere_responses].
...
improvements in much more powerful and general languages, such as Python and
R.  These languages are particularly suitable for beginners in
data analysis, and they come with a huge range of tools and libraries for a
very wide range of tasks in data analysis, including the kinds of models and
simulations you will see in this book.  We have therefore updated the book to
use Python, instead of *Resampling Stats*.  If you already know Python or a
similar language, such as R, you will have a big head start in
reading this book, but even if you do not, we have written the book so it will
be possible to pick up the Python code that you need to understand and build
the kind of models that Simon uses.  The advantage to us, your authors, is
that we can use the very powerful tools associated with Python to make it
easier to run and explain the code.  The advantage to you, our readers, is
that you can also learn these tools, and the Python language, and continue to
use these tools for the rest of your career in data analysis.
...
has passed since he wrote the book, for the reasons we discuss above, his
tools and explanation have reached the mainstream.  It now perfectly possible
to teach an introductory statistics course without referring to the older
statistical methods.  This means that the earlier editions of this book can
now serve on their own as an introduction to statistics --- but, used this
way, at the time we write, this will leave our readers with some gaps to fill.
They should have a deep understanding of the method of comparing measures from
two groups, using resampling methods, but they are likely to come across other
teachers and researchers using more traditional methods such as t-tests.  To
bridge this gap, we have added material to many chapters that explain how the
resampling methods relate to the traditional methods.  Luckily, we find these
explanations add deeper understanding to the traditional methods. Teaching
resampling is the best way to teach t-tests, and other traditional methods.
...
This book teaches the basics of using the Python language, basic probability,
...
reorganizing data using Python, or another language, such as R. It
